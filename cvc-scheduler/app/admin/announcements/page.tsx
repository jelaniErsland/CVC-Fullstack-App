...
export default async function CommunicationsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (!loaded || loaded[1].error) return <AdminShell active="announcements" workspaceName={context.workspace.displayName}><h1 className="text-3xl font-bold">Communications</h1><p className="mt-4 text-slate-600">Delivery history is temporarily unavailable. Refresh before sending anything.</p></AdminShell>;
  const [history, pending] = loaded;
    ...
    return <AdminShell active="announcements" workspaceName={context.workspace.displayName}>
      <header><p className="text-xs font-bold uppercase tracking-wider text-blue-600">{context.workspace.displayName}</p><h1 className="mt-1 text-3xl font-bold tracking-tight">Communications</h1></header>
