...
async function verifyPreflightAndReadAnonKey() {
  assert(
    process.env.RUN_HOSTED_AUDITED_RESPONSE_REVEAL_VALIDATION === expectedConfirmation,
    `Refusing hosted validation without RUN_HOSTED_AUDITED_RESPONSE_REVEAL_VALIDATION=${expectedConfirmation}.`,
  );
  ...
  assert(
    latestMigration === "20260704000000",
    "Hosted staging is not migrated through 20260704000000.",
  );
...
async function createFixtures(password) {
  ...
  const workspaceName = "QA 11.24 Hosted Audited Reveal Workspace";
  const taskTitle = "QA 11.24 Hosted Audited Reveal Assignment";
  const questionnaireAnswers = JSON.stringify({
    aboutYou: {
      name: "QA 11.24 Disposable Volunteer",
      email: `${fixture.namespace}-volunteer@example.invalid`,
      phone: "+1 555 011 2419",
      congregation: "QA 11.24 Hosted Test",
    },
    availability: { weekdays: ["Tuesday"], preferredTimes: ["Morning"] },
    skillsExperience: { categories: ["General"], maintenanceTaskCards: false },
    emergencyContact: { name: "QA Disposable Contact", phone: "+1 555 011 2499" },
    otherWaysToHelp: {},
  });
...
insert into public.volunteer_profiles (
  ...
  'QA 11.24 Disposable Volunteer', '${fixture.namespace}-volunteer@example.invalid',
  '+1 555 011 2419', 'QA 11.24 Hosted Test',
...
async function verifyAuditedReveal(anonKey) {
  const publicClient = createClient(hostedUrl, anonKey, {
    auth: { autoRefreshToken: false, detectSessionInUrl: false, persistSession: false },
  });
  const unauthenticated = await publicClient.rpc("reveal_assignment_response_link", {
    p_assignment_id: fixture.assignmentId,
    p_ttl_hours: 72,
    p_reveal_mode: "copy_link",
    p_metadata: {},
  ...
  assert(
    unauthenticated.error?.code === "42501" && unauthenticated.data === null,
    "Hosted audited reveal succeeded without real Auth.",
  );
  ...
  assert(!setup.error && setup.data?.length === 1, "Hosted audited-reveal setup failed.");
  const oldTokenId = setup.data[0]?.token_id;
  const oldBearer = setup.data[0]?.bearer_token;
  assert(uuidPattern.test(oldTokenId) && bearerPattern.test(oldBearer), "Hosted audited-reveal setup token was invalid.");
  secrets.add(oldBearer);

  const stateBeforeDenial = runHostedSql(`select
  (select count(*) from public.assignment_response_tokens where workspace_id = '${fixture.workspaceId}'::uuid) as token_count,
  (select count(*) from public.assignment_response_link_reveal_events where workspace_id = '${fixture.workspaceId}'::uuid) as audit_count,
  (select count(*) from public.assignment_response_tokens where workspace_id = '${fixture.workspaceId}'::uuid and revoked_at is null) as active_count;`, "Hosted audited-reveal baseline state")[0];
...
select 'restricted' as grant_state;`);
  const denied = await authenticatedClient.rpc("reveal_assignment_response_link", {
    ...
    p_ttl_hours: 72,
    p_reveal_mode: "copy_link",
    p_metadata: { reason_code: "hosted_qa" },
  });
  assert(
    denied.error?.code === "42501" && denied.data === null,
    "Hosted audited reveal bypassed assignments.edit.",
  );
  const stateAfterDenial = runHostedSql(`select
  (select count(*) from public.assignment_response_tokens where workspace_id = '${fixture.workspaceId}'::uuid) as token_count,
  (select count(*) from public.assignment_response_link_reveal_events where workspace_id = '${fixture.workspaceId}'::uuid) as audit_count,
  (select count(*) from public.assignment_response_tokens where workspace_id = '${fixture.workspaceId}'::uuid and revoked_at is null) as active_count;`, "Hosted audited-reveal denied state")[0];
  assert(
    JSON.stringify(stateAfterDenial) === JSON.stringify(stateBeforeDenial),
    "Denied hosted audited reveal mutated token or audit state.",
  );
  const oldAfterDenial = await publicClient.rpc("read_assignment_response_by_token", {
    p_bearer_token: oldBearer,
  });
  assert(!oldAfterDenial.error && oldAfterDenial.data?.length === 1, "Denied hosted audited reveal revoked the old token.");
...
select 'restored' as grant_state;`);
  ...
  assert(
    rejected.every((result) => result.error?.code === "42501" && result.data === null),
    "Hosted audited reveal accepted an invalid TTL, mode, or metadata object.",
  );
  const stateAfterRejected = runHostedSql(`select
  (select count(*) from public.assignment_response_tokens where workspace_id = '${fixture.workspaceId}'::uuid) as token_count,
  (select count(*) from public.assignment_response_link_reveal_events where workspace_id = '${fixture.workspaceId}'::uuid) as audit_count,
  (select count(*) from public.assignment_response_tokens where workspace_id = '${fixture.workspaceId}'::uuid and revoked_at is null) as active_count;`, "Hosted audited-reveal rejected-input state")[0];
  assert(
    JSON.stringify(stateAfterRejected) === JSON.stringify(stateBeforeDenial),
    "Rejected hosted audited reveal input mutated token or audit state.",
  );

  const correlationId = randomUUID();
  const valid = await authenticatedClient.rpc("reveal_assignment_response_link", {
    p_assignment_id: fixture.assignmentId,
    p_ttl_hours: 72,
    p_reveal_mode: "copy_link",
    p_metadata: {
  ...
  assert(!valid.error && valid.data?.length === 1, "Hosted valid audited reveal failed.");
  const responseTokenId = valid.data[0]?.response_token_id;
  const auditEventId = valid.data[0]?.audit_event_id;
  const expiresAt = valid.data[0]?.token_expires_at;
  const bearer = valid.data[0]?.bearer_token;
  assert(
    uuidPattern.test(responseTokenId) &&
      uuidPattern.test(auditEventId) &&
      bearerPattern.test(bearer) &&
      Number.isFinite(Date.parse(expiresAt)) &&
      valid.data[0]?.event_reveal_surface === "future_project_contact_assignment_response_reveal" &&
      valid.data[0]?.event_reveal_mode === "copy_link",
    "Hosted audited reveal returned an unsafe or invalid result shape.",
  );
  secrets.add(bearer);
  ...
  assert(
    ...
    "Hosted audited reveal left the old bearer usable.",
  );
  const newVerification = await publicClient.rpc("read_assignment_response_by_token", {
    p_bearer_token: bearer,
  });
  assert(!newVerification.error && newVerification.data?.length === 1, "Hosted audited-reveal bearer did not verify publicly.");

  const stored = runHostedSql(`select
  (select revoked_at is not null from public.assignment_response_tokens where id = '${oldTokenId}'::uuid) as old_revoked,
  (select octet_length(token_verifier_hash) from public.assignment_response_tokens where id = '${responseTokenId}'::uuid) as verifier_bytes,
  (select count(*) from public.assignment_response_link_reveal_events where id = '${auditEventId}'::uuid) as event_count,
  workspace_id,
  assignment_id,
  response_token_id,
  actor_project_contact_id,
  reveal_surface,
  reveal_mode,
  expires_at = '${expiresAt}'::timestamptz as expiry_matches,
  metadata,
  (select count(*) from information_schema.columns
   where table_schema = 'public'
     and table_name = 'assignment_response_link_reveal_events'
...
from public.assignment_response_link_reveal_events
where id = '${auditEventId}'::uuid;`, "Hosted audited-reveal persistence check")[0];
  assert(
    stored?.old_revoked === true &&
      stored?.verifier_bytes === 32 &&
      stored?.event_count === 1 &&
      stored?.workspace_id === fixture.workspaceId &&
      stored?.assignment_id === fixture.assignmentId &&
      stored?.response_token_id === responseTokenId &&
      stored?.actor_project_contact_id === fixture.contactId &&
      stored?.reveal_surface === "future_project_contact_assignment_response_reveal" &&
      stored?.reveal_mode === "copy_link" &&
      stored?.expiry_matches === true &&
      stored?.metadata?.reason_code === "hosted_qa" &&
      stored?.metadata?.delivery_requested === false &&
      stored?.metadata?.request_correlation_id === correlationId &&
      stored?.forbidden_columns === 0,
    "Hosted audited reveal did not persist one correctly scoped credential-free audit event.",
  );

  const newSubmission = await publicClient.rpc("submit_assignment_response_by_token", {
    p_bearer_token: bearer,
    p_response_status: "declined",
    p_response_note: null,
  });
  assert(
    ...
    "Hosted audited-reveal bearer could not submit a public response.",
  );
  ...
  const concurrent = await Promise.all([
    authenticatedClient.rpc("reveal_assignment_response_link", {
      ...
      p_ttl_hours: 72,
      p_reveal_mode: "copy_link",
      p_metadata: { reason_code: "hosted_concurrency" },
    }),
    authenticatedClient.rpc("reveal_assignment_response_link", {
      p_assignment_id: fixture.assignmentId,
      p_ttl_hours: 72,
  ...
  const concurrentResults = concurrent.map((result, index) => {
    assert(!result.error && result.data?.length === 1, `Hosted concurrent audited reveal ${index + 1} failed.`);
    const concurrentBearer = result.data[0]?.bearer_token;
    assert(bearerPattern.test(concurrentBearer), `Hosted concurrent audited reveal ${index + 1} returned an invalid bearer.`);
    secrets.add(concurrentBearer);
    return {
      tokenId: result.data[0]?.response_token_id,
      auditId: result.data[0]?.audit_event_id,
      bearer: concurrentBearer,
    };
  });
  const concurrentVerification = await Promise.all(
    concurrentResults.map((entry) =>
      publicClient.rpc("read_assignment_response_by_token", { p_bearer_token: entry.bearer }),
    ),
  );
  assert(
    concurrentVerification.filter((result) => !result.error && result.data?.length === 1).length === 1,
    "Hosted concurrent audited reveals left more than one usable bearer.",
  );
  const concurrencyState = runHostedSql(`select
  (select count(*) from public.assignment_response_tokens where workspace_id = '${fixture.workspaceId}'::uuid and assignment_id = '${fixture.assignmentId}'::uuid and revoked_at is null and expires_at > clock_timestamp()) as active_tokens,
  (select count(*) from public.assignment_response_link_reveal_events where id in ('${concurrentResults[0].auditId}'::uuid, '${concurrentResults[1].auditId}'::uuid) and response_token_id in ('${concurrentResults[0].tokenId}'::uuid, '${concurrentResults[1].tokenId}'::uuid)) as coupled_audits;`, "Hosted audited-reveal concurrency state")[0];
  assert(
    concurrencyState?.active_tokens === 1 && concurrencyState?.coupled_audits === 2,
    "Hosted concurrent audited reveals did not leave one active token and one audit per successful command.",
  );
}
...
function verifyNamespaceResidue() {
  ...
  (select count(*) from public.workspaces where workspace_key like 'qa-11-24-%') +
  (select count(*) from auth.users where email like 'qa-11-24-%@example.invalid')
) as residue_count;`, "Hosted 11.24 namespace residue check")[0]?.residue_count;
  assert(residue === 0, "Hosted qa-11-24 namespace left fixture or Auth residue.");
  return residue;
}
...
async function main() {
  ...
  try {
    ...
    await verifyAuditedReveal(anonKey);
  } finally {
  ...
  console.log("Hosted staging transactional audited-response-link reveal QA passed.");
  console.log("Validated migration 20260704000000 against the approved non-production staging ref.");
  console.log("Verified rollback, Auth/capability enforcement, atomic replacement, one-audit coupling, public use, hash-only storage, and concurrent single-active-token behavior.");
...
main().catch((error) => {
  console.error(`Hosted transactional audited-response-link reveal QA failed: ${redact(error)}`);
  process.exitCode = 1;
});
