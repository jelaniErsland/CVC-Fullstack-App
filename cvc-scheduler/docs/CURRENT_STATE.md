...
- The persisted schema boundaries are workspace identity, project contacts/grants, immutable questionnaire truth, volunteer profile snapshots, and reusable task presets. They include no seed data or browser-side table writes.
- Generated database types and live schema validation remain pending a configured linked/local Supabase database.
- Capability enforcement additionally covers `tasks.view` and `tasks.edit`; roles and extra capability names do not authorize future product data.
...
11.9 Calendar Item Persistence, if separately approved. Keep scheduled occurrences distinct from task presets and initially add no assignment or volunteer-response mutation.
