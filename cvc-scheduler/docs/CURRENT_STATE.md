...
Review the unlinked assignment-detail shell’s visual and unavailable-state behavior, then decide whether the next slice should remain QA-only or implement the still-unlinked POST action with every product-action/UI/reveal availability flag false. Keep product navigation, copy UI, delivery, and mock-route cutovers disabled.
