...
Next work may continue hardening the disabled assignment-detail wiring proof or add more disabled-only harness coverage, but `/admin/assignments/[assignmentId]` must not call the server-action stub, import the adapter/product-action/reveal helpers directly, or expose any enabled response-link action until a later reviewed slice explicitly proves final approval, POST-only wiring, warning/expiration UI, post-success-only manual copy, and credential/log/browser guardrails.
