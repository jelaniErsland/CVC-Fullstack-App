...
The direct response shell and server-only project-contact link issuer now have repeatable disposable local gates. Before any visible link-generation or delivery surface, review transport, abuse/rate-limit, operational revocation, and audit requirements separately. Keep the mock volunteer portal, Calendar, Volunteers, Communications, and Needs Attention route cutovers—plus email/reminder delivery, broad lookup, remembered devices, and admin UI—in later reviewed slices.
