...
The direct response shell now has a repeatable disposable local valid-token gate. Before any link delivery, review transport, abuse/rate-limit, and operational revocation requirements separately. Keep the mock volunteer portal, Calendar, Volunteers, Communications, and Needs Attention route cutovers—plus email/reminder delivery, broad lookup, remembered devices, and admin link generation—in later reviewed slices.
