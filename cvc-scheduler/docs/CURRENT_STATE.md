...
Implement and review an unlinked, dynamic/no-store `/admin/assignments/[assignmentId]` route shell that reads only `readAssignmentDetailContext`, presents one uniform unavailable state, and contains no response-link action. Keep product navigation, the audited reveal action, copy UI, and all mock-route cutovers disabled until separately reviewed.
