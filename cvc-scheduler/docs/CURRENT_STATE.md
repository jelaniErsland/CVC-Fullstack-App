...
Next work should be `12.4 Route-Unused Calendar Read Helper QA Harness or Disposable Local Data Validation` only if the 12.3 helper/query-shape remains clean; otherwise revise 12.3 first. Do not cut over `/admin/calendar`, add Calendar writes, assignment picker UI, assignment-detail entry links, delivery, public lookup, remembered devices, service-role usage, seed data, mock-to-real mixing, or response-link activation.
