...
Next work may continue hardening the route-unused server-action seam or review a still-disabled form wiring plan, but `/admin/assignments/[assignmentId]` must not import the server-action stub/adapter or expose a working response-link action until a later reviewed slice explicitly proves final approval, POST-only wiring, warning/expiration UI, post-success-only manual copy, and credential/log/browser guardrails.
