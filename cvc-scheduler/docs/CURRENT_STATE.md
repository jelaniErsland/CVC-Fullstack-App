...
Next work should be `12.5 Route-Unused Calendar Read Model Disposable Local Data Validation` only if the 12.4 in-memory QA harness remains clean; otherwise revise 12.4 first. Do not cut over `/admin/calendar`, add Calendar writes, assignment picker UI, assignment-detail entry links, delivery, public lookup, remembered devices, service-role usage, seed data, hosted validation, mock-to-real mixing, or response-link activation.
