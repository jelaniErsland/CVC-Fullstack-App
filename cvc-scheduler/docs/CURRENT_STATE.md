...
Calendar interaction regression coverage is available through `npm run test:calendar` while an app preview is already running. The focused Playwright script checks desktop and 390px mobile keyboard activation for view switching, Week/List navigation, Day/Month arrow movement and creation, Food filtering, inspector and creation focus/Escape restoration, dialog descriptions and Tab/Shift+Tab containment, Month sibling/overflow semantics, List rows, mobile bottom navigation/More, overlay exclusivity, browser errors, hydration warnings, and horizontal overflow. It first checks that the target is reachable, then reports scoped step timing plus URL, viewport, pressed-view, focus, and dialog context when an interaction fails.
...
- Week remains on normal document Tab order and has no hour-by-hour arrow model; List remains a normal button list. Day/Month arrow movement is an initial local focus helper, not a complete ARIA calendar-grid or roving-focus implementation. Automated semantic QA is not a substitute for a multi-screen-reader/device audit.
