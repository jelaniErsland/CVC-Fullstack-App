...
Next work may stay in route-unused server-action/form planning or separately review activation prerequisites, but `/admin/assignments/[assignmentId]` must not import the disabled adapter or expose a working response-link action until a later reviewed slice explicitly proves final approval, POST-only wiring, warning/expiration UI, post-success-only manual copy, and credential/log/browser guardrails.
