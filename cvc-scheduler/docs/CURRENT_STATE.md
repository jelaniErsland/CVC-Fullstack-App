...
- The all-day band recognizes future explicit `allDay` items, `All day` time windows, or untimed items; supports optional `endDate` multi-day spans; shows up to two compact lanes; and provides a quiet per-day `+N` overflow action that focuses the existing Day view. Current mock Calendar data is entirely timed, so the band is intentionally empty today.
