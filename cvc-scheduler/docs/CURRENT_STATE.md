...
The direct response shell, server-only link issuer, and unlinked redacted admin diagnostic now exist. Before any real product link-generation or delivery surface, review transport, abuse/rate-limit, operational revocation, unused-token cleanup, and audit requirements separately. Keep the mock volunteer portal, Calendar, Volunteers, Communications, and Needs Attention route cutovers—plus email/reminder delivery, broad lookup, remembered devices, and polished admin UI—in later reviewed slices.
