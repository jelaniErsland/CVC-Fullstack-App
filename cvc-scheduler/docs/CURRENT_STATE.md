...
Next work should be `12.2 Persisted Calendar Read Model Contract`: define a route-unused, server-only Calendar read model for persisted `calendar_items`, assignment-derived counts, and safe workspace/contact grants without cutting over `/admin/calendar`. Response-link activation remains paused; do not add active reveal, copy UI, delivery, public lookup, remembered devices, or response-link product navigation unless a later explicit slice reopens that ladder.
