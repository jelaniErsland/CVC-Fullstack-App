...
The preferred independent encrypted backup and full technical recovery path, recurring task registration, safe human-confirmed failure notification, and recovery/rollback ownership are proven. Before real Bozeman product data is provisioned, preserve that path and prove one successful scheduled backup/checksum/retention execution after diagnosing `migration_preflight_failed`. The Supabase-managed Pro path remains optional.
