...
After 12.11, `npm run test:calendar` validates the cut-over route against disposable local persisted fixtures while a loopback production preview is running. It refuses non-loopback targets, authenticates project contacts, renders persisted success, empty, and under-capability unavailable states, checks Day/Week/Month/List switching, date navigation, filters, inspector behavior, 390px layout, no unsafe leakage, no mock item leakage, and cleans every fixture in `finally`. After 12.16 it also performs a persisted one-off timed create -> reload -> edit -> reload browser round trip. After 12.17 it additionally exercises the real persisted task-preset selector and preset-backed timed create -> reload -> edit -> reload path. Start preview with logs redirected to temp files, do not print raw Supabase/preview env output, and stop preview before final `npx tsc --noEmit` or `npm run build`.
