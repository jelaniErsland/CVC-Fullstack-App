...
- Reconciled the volunteer response as a human-performed action: the product owner opened the real email, followed its normal schedule link, and personally clicked Confirm exactly once before Codex preflight. Codex clicked Confirm zero times. Authoritative production persisted `confirmed` with `volunteer_schedule` provenance. The separate Codex browser did not carry the product owner's HttpOnly volunteer-session cookie, which is expected browser-session isolation and not a product failure.
- Verified the confirmed response through normal authenticated admin product routes. Calendar and Assignment Detail showed Confirmed, the item stayed published with one active assignment and derived `1/1` coverage, the former pending-response Needs Attention signal cleared naturally, and Overview remained coherent. Reloaded Initial Email state showed `Ready 0 / Already sent 1` with the ordinary send action disabled; no second provider attempt was made.
- Invoked the production Notification Health route exactly once through the authorized Bozeman admin session. It reported healthy with no stale assignment email deliveries and performed zero send, retry, reclaim, finalize, or product mutation. Removed only the temporary production email transport variable, waited for the disabled Production deployment to become Ready/Current, and reverified one successful external delivery, one hash-only schedule credential, zero stale/failed/sending rows, zero assignment-response tokens, the confirmed published `1/1` assignment, migration `20260824123500`, and the enabled/Ready/not-running backup task at the matching lock. The production scheduling loop is now end-to-end proven; a tiny controlled Bozeman pilot and explicit launch decision remain next.
...
- Disabled the non-running permanent backup task for the reviewed migration window and applied only `20260824123500_follow_up_contact_self_edit.sql` to exact production `project-local-production` (`wdlaauzknfggoqldolmx`). Production advanced from `20260812123430` to `20260824123500`. Postflight passed RPC/signature/search-path/security-definer metadata, unauthenticated functional denial, direct authenticated contact-table UPDATE denial, generated public-schema type parity, all-table RLS, the same four FORCE RLS tables, and the established direct-table privilege posture.
- Preserved the single Auth identity, active Bozeman workspace/contact/effective grant, ready volunteer, active General task preset, published Calendar item, active assignment, `needs_response`, `1/1` coverage, Follow-up Contact association, and zero delivery/token state. Only the permanent backup task migration lock advanced from `20260812123430` to `20260824123500`; it was re-enabled Ready/not-running with its reviewed action, principal, schedule, notification, secret-handling, and next normal occurrence unchanged and no catch-up/manual execution.
- Used only the real authenticated production Calendar self-edit form to populate the current contact's approved volunteer-facing details; no personal values are recorded in repository documentation. Blank phone persisted as null, and reload proved Initial Email eligibility changed from `Ready 0 / Needs Follow-up Contact 1` to `Ready 1 / Needs Follow-up Contact 0`; `Already sent` and `Missing email` remained zero. Application email transport stayed disabled, and delivery claims, provider sends, schedule-access credentials, assignment-response tokens, volunteer-link exercise, volunteer responses, and Notification Health calls remained zero. The explicit 12.43B human send checkpoint remains next.

## Iteration 12.43B.1 - Volunteer-Facing Follow-up Contact Self-Edit Boundary

- The 12.43B production preflight established that the controlled Calendar item's Follow-up Contact already resolves to its authenticated creator/contact, but there was no reviewed product path for that contact to maintain the existing volunteer-facing name, email, and phone fields. Email sending remained paused.
- Added migration `20260824123500_follow_up_contact_self_edit.sql` with the narrow authenticated `update_current_project_contact_volunteer_facing_details` RPC. It derives identity from `auth.uid()`, requires exactly the caller's active contact with an effective `workspace.read` grant in the active trusted workspace, updates only that contact's three bounded volunteer-facing fields, normalizes blank phone to null, and returns only boolean success. It adds no column, table, RLS, direct table privilege, service-role path, email, or token change; direct authenticated contact-table UPDATE remains denied.
- Added a fresh verified server action and compact Add/Edit form inside the persisted Calendar Initial Email area only when the selected item's Follow-up Contact is the current authenticated contact. The browser supplies only name, email, optional phone, and safe redirect metadata; it supplies no workspace/contact/capability id. Mock Settings remains untouched.
- Disposable local DB and desktop/390px browser validation proves normalized own-contact save/reload, unauthenticated/no-grant/revoked/expired/future/inactive-contact/inactive-workspace/cross-workspace/malformed denial, cross-contact isolation, protected scheduling/auth/grant preservation, direct-table denial, and Initial Email eligibility changing from `Ready 0 / Needs Follow-up Contact 1` to `Ready 1 / Needs Follow-up Contact 0`. Email transport stayed disabled; delivery claims, provider sends, schedule-access credentials, response tokens, and Notification Health calls remained zero.
- After the approved non-production project was reactivated, exact-target hosted validation verified `ACTIVE_HEALTHY` `project-local-staging` (`kfuujcfxoayukywvtaeh`) at `20260812123430`, applied only `20260824123500_follow_up_contact_self_edit.sql`, and proved hosted generated public-schema type parity, own-contact update and blank-phone normalization, every required authorization/lifecycle/isolation/malformed/direct-write denial, protected scheduling/Auth/grant preservation, the `Ready 0 / Needs Follow-up Contact 1` to `Ready 1 / Needs Follow-up Contact 0` transition, disabled transport, zero provider/Notification Health activity, and exact-run/namespace/Auth fixture residue `0`. No production access, mutation, contact-value transmission, or email occurred. The implementation remains uncommitted for product-owner visual review; the controlled production email proof remains paused.
...
- Added the canonical core signals only: published active underfilled work within 14 days, pending current responses within 21 days, and denied current responses within 21 days. Confirmed and pending assignments count toward coverage, denied and canceled assignments do not, and draft, passed, canceled, fully covered, zero-needed, outside-window, and reconfirmed/current-confirmed cases clear or avoid signals naturally.
- Added deterministic workspace-timezone urgency (`Within 48 hours`, `Within 7 days`, `Upcoming`), chronological/type/stable-key ordering, a 100-signal maximum, Staffing/Responses grouping, compact expandable rows, safe coverage detail, Calendar date navigation, calm unavailable/error/all-caught-up states, and no raw ids/contact/token/provider/grant output.
- Promoted Needs Attention into the desktop primary sidebar and canonical mobile fourth tab while moving Volunteers into mobile More. Added focused pure/static and real-route browser regressions plus six disposable 1440px/390px review captures with zero fixture/Auth residue. The historical mock detail route remains isolated and no real row links to it.
- No Needs Attention row/table, dismissal/resolution/history state, conflict/availability heuristic, direct write, GET mutation, migration, generated type, SQL/RPC/RLS/Auth/token change, service-role path, hosted access, production/staging, backup task, email, or real Bozeman data is part of this review checkpoint. Product-owner functional and visual review remains pending; launch remains `NO-GO`.
...
- Added a Calendar-first Project Local desktop/mobile composition, authorized quick actions, calm partial-capability behavior, and one intentional ready-empty project state. Disposable persisted browser fixtures produced five 1440px/390px review captures with zero residue; Jelani approved the functional and visual result and 12.38.1 committed the reviewed cutover.
...
- No migration, generated type, SQL/RPC/table/view, schema/RLS/Auth/token, production/staging, backup task, email, or real Bozeman data was part of the 12.38 cutover. Communications, the then-dedicated Needs Attention surface, and `/v/demo` remained prototypes at that checkpoint. Launch remained `NO-GO`.
