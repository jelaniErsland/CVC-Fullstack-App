...
Local validation passed through `npm run test:volunteer-schedule-access` and `npm run test:volunteer-schedule-access:browser`, proving hash-only issuance, TTL bounds, publication-only reads, ready/empty/unavailable states, revocation, cookie security, no mock leakage, no unsafe output, direct table denial, isolation, and zero residue. Because 12.20 adds migration `20260714122000_volunteer_schedule_access.sql`, RPCs, and generated public-schema type changes, hosted non-production validation is required next before beta reliance. The recommended next gate is `12.20.1 Hosted Staging Volunteer Schedule Access Validation Gate` against `project-local-staging` (`kfuujcfxoayukywvtaeh`).
