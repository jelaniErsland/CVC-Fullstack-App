...
Local validation uses `npm run test:assignment-notification-email`. 12.22.1 completes the required hosted non-production validation gate on `project-local-staging` (`kfuujcfxoayukywvtaeh`), verified as `ACTIVE_HEALTHY`. Staging is now validated through migration `20260714122230`: `20260714122200_initial_assignment_notifications.sql` was applied, then hosted-discovered malformed-recipient and finalization-bound defects were corrected with forward migrations `20260714122210`, `20260714122220`, and `20260714122230` without rewriting hosted migration history.
...
12.23.1 adds the integrated hosted end-to-end beta-loop gate, `npm run test:bozeman-beta-e2e:hosted`, exact-opt-in locked by `RUN_HOSTED_BOZEMAN_BETA_E2E_VALIDATION=project-local-staging:kfuujcfxoayukywvtaeh`. It creates one disposable hosted namespace only after verifying the approved non-production target, health, migration `20260714122230`, and generated public-schema type parity. The successful gate exercised normal Auth/session and product routes across Volunteers Add/Edit, Calendar scheduling, assignment, publication, recording-only Initial email, secure volunteer schedule handoff, Confirm/Deny/Confirm All, admin response truth, negative access/prerequisite paths, safe-output checks, and cleanup. Exact-run, namespace, and Auth residue counts were `0`. It does not change schema/RLS/RPCs/generated types, use service-role application behavior, target production, send real email, or activate response-link reveal/copy.

## 12.24 Production environment readiness

12.24 adds a server-only, route-unused production readiness contract at `lib/readiness/productionEnvironmentReadiness.server.ts` plus `npm run test:production-environment-readiness`. It does not create a production Supabase project, apply production migrations, deploy production, configure DNS, create real Bozeman rows, configure a real email provider, send email, use service-role application behavior, activate response-link reveal/copy, or change generated public-schema types.

The contract records `NO-GO`, current production migration `20260812123430`, strict separation from staging `project-local-staging` (`kfuujcfxoayukywvtaeh`), and Vercel as the recommended first production host based on the current plain Next.js repository shape. `docs/PRODUCTION_ENVIRONMENT_INVENTORY.md`, `docs/PRODUCTION_DEPLOYMENT_RUNBOOK.md`, and `docs/JELANI_PRODUCTION_SETUP_CHECKLIST.md` document the completed production schema transition, Auth Site URL and redirect allowlist, domain/DNS planning, environment variables, disabled email transport, no service-role key, read-only smoke-test design, observability, backups, rollback, and operator handoff. Remaining provisioning, application-email proof, first real Notification Health execution, and pilot work keep launch at `NO-GO`.

## 12.25 Production Supabase schema gate

12.25 adds `scripts/production-supabase-schema-regression.mjs`, `npm run test:production-supabase-schema`, and `docs/PRODUCTION_SUPABASE_STATUS.md` for the first controlled production database schema operation. The gate is the initial/bootstrap empty-production schema gate: it is locked to `project-local-production` (`wdlaauzknfggoqldolmx`), forbids staging ref `kfuujcfxoayukywvtaeh`, expects terminal migration `20260714122230`, and allows only reviewed migration application plus read-only schema/type/security/count validation. Fixtures, Auth users, product rows, storage objects, email transport, service-role runtime configuration, response-link reveal/copy, production deployment, DNS/Auth redirect changes, and real Bozeman data remain forbidden.

The production bootstrap gate passed against the exact approved target. The first run refused safely while the CLI was still linked to staging; after relinking to production, the dry-run showed only reviewed committed migrations through `20260714122230` with no seeds or roles, the migration chain applied, and the committed gate verified `ACTIVE_HEALTHY`, final migration `20260714122230`, generated public-schema type parity, zero product/Auth/storage rows before Auth setup, public Supabase connectivity, 13 RLS-protected product tables, and 0 broad direct mutation grants. That bootstrap evidence remains historical after 12.26 manual Auth evidence. 12.36.5 then completed the established-production transition through `20260811123300` and `20260812123430`; production postflight passed and the backup lock is now `20260812123430`. Launch remains `NO-GO` pending provisioning, controlled application-email proof, first real Notification Health execution, and pilot evidence.

12.25.1 stabilizes the pristine-project migration-history check. The gate now tests whether `supabase_migrations.schema_migrations` exists before selecting from it, treats a missing table as clean initial history only after the exact production target/environment guards pass, and continues to fail closed for actual query failures, malformed migration versions, and unexpected partial remote history. `npm run test:production-supabase-schema:pristine` protects this edge case.

## 12.26 Production deployment/Auth smoke gate

12.26 records the first production deployment/Auth evidence without adding product features or data. Vercel project `project-local` is live at `https://project-local-one.vercel.app`; production uses `ADMIN_AUTH_MODE=enforced`; production Supabase public URL/key are configured in Vercel Production only; `SUPABASE_SERVICE_ROLE_KEY`, email transport, and recording transport are absent; Supabase Auth Site URL is `https://project-local-one.vercel.app`; and the exact callback is `https://project-local-one.vercel.app/admin/auth/callback`.

12.27 records the final production domain/Auth evidence and retargets the public smoke gate. Canonical production origin is `https://projectlocal.app`; the temporary Vercel alias `https://project-local-one.vercel.app` remains available with its callback allowlisted for fallback; Supabase Auth Site URL is `https://projectlocal.app`; exact final-domain callback is `https://projectlocal.app/admin/auth/callback`; HTTPS loaded without browser warning; manual magic-link sign-in returned through the final domain; the signed-in no-grant user opened the admin shell and failed closed on `/admin/calendar` and `/admin/volunteers`; and no Project Local product rows were created.

`npm run test:production-deployment-smoke` is exact-opt-in only and public HTTP-only. It verifies the landing page, anonymous admin redirect, login page render, invalid volunteer access redirect, unauthenticated schedule unavailable state, same-origin redirects, volunteer-route privacy headers, and safe output against `https://projectlocal.app`. It does not request a magic link, create Auth users, create workspace/contact/grant rows, create fixtures, send email, call Vercel APIs, call Supabase mutation APIs, configure DNS/Auth URLs, or use service-role application behavior. Missing/wrong opt-in and dirty-worktree refusal paths passed during implementation; the same public HTTP assertions passed through a separate non-mutating diagnostic; commit `082c960` was pushed to `origin/master`; the Vercel Production deployment sourced from `082c960` reached Ready; and the exact final-domain production deployment smoke passed again with exit code `0`.

Manual operator evidence, recorded in `docs/PRODUCTION_DEPLOYMENT_STATUS.md`, confirms that an existing approved Auth email could sign in by magic link, return through the final-domain production callback, open the admin shell, and fail closed on `/admin/calendar` and `/admin/volunteers` because no Project Local workspace/contact/grant exists yet. Jelani explicitly product-owner approved the 12.30.1 beta-critical UI and all six desktop/390px review captures. August 10, 2026 operator evidence separately proves Resend provider/domain/sender configuration and direct provider-level inbox delivery, while Project Local application email remains disabled and unproven. Launch remains `NO-GO` until application-driven email/monitoring, backup/restore, observability, real operator provisioning, and pilot evidence pass.

## 12.28 Production backup/recovery readiness

12.28 adds `lib/readiness/productionRecoveryReadiness.server.ts`, `npm run test:production-recovery-readiness`, and [`PRODUCTION_BACKUP_RECOVERY_RUNBOOK.md`](./PRODUCTION_BACKUP_RECOVERY_RUNBOOK.md). This is a server-only, route-unused documentation/readiness boundary; it does not access production Supabase, link the CLI, perform a restore, create a dump, request a magic link, mutate Vercel/DNS/Auth/email settings, create data, send email, add migrations, or change generated types.

The runbook distinguishes application rollback from database rollback, records commit `082c960` as the verified production deployment baseline, preserves migration-forward recovery, documents grant-revocation/email-disable pause and Belgrade fallback, and defines safe post-recovery verification. 12.28.1 records Free-plan managed-backup limitations; Supabase Pro remains optional and PITR unnecessary for the initial beta. 12.34 proves the first independent encrypted production backup, checksum/status, and retention behavior. 12.34.1 verifies managed platform roles rather than recreating them, 12.34.2 attributes the 26 direct `TRUNCATE` grants to restore interaction, and 12.34.3 reconstructs the historical source ACL deterministically and completes local recovery-forward through `20260812123430`. 12.35 proves recurring task registration, safe human-visible failure notification, and recovery/rollback ownership. 12.35.11 proves successful scheduled-host production backup/checksum/retention, and 12.35.12 safely enables the permanent task without catch-up execution. Backup/recovery is complete and non-blocking. The logical database package does not prove Supabase Auth platform configuration or Storage object BLOB recovery. Application email remains disabled and launch remains `NO-GO`.

## 12.31 Production transactional email provider integration

12.31 selects Resend and adds a direct server-only HTTPS adapter behind the existing 12.22 initial-assignment notification abstraction. The boundary now recognizes explicit disabled, recording, and Resend modes. Resend mode requires a valid trusted notification origin, approved sender address, and server-only `RESEND_API_KEY`; missing, malformed, or unknown configuration fails closed and never falls back to recording. The API key never uses `NEXT_PUBLIC_`, enters Supabase, renders, or appears in safe output.

The adapter sends only the existing persisted-truth message contract and keeps the schedule bearer/full access URL in memory for the provider request. It derives a deterministic provider idempotency header by hashing the database-issued non-secret ledger idempotency key; the database claim/finalize ledger remains authoritative because Resend's provider idempotency window is limited. Only the existing bounded provider message-id shape may be persisted. Non-2xx, network, JSON, or malformed-id failures collapse to `provider_send_failed`; raw provider bodies and request credentials are never returned or stored.

`npm run test:assignment-notification-email:resend` uses injected fake HTTP responses and no provider network. It covers configuration, request shape, Project Local sender/recipient/content mapping, schedule-link inclusion in the outbound body but exclusion from returned/recorded/browser-safe output, deterministic non-secret idempotency, safe/malformed provider ids, non-2xx and network failures, and preservation of recording transport. The existing `npm run test:assignment-notification-email` continues to prove the Supabase claim/finalize ledger, duplicate prevention, failure retry, stale recovery, schedule-access secrecy, and zero local residue. No schema, migration, RPC, RLS, generated type, Auth, token/cookie, hosted Supabase, real email, provider/DNS/Vercel setting, or UI changed.

Application integration is not activation evidence. Resend domain and sender verification, a least-privilege API key in encrypted Vercel server settings, canonical base URL, privacy-first tracking settings, an approved test-recipient policy, credential-free monitoring, and real deliverability proof remain required. Production email and launch remain `NO-GO` until those gates pass.

## 12.31.1 Resend operator evidence without persistence access

August 10, 2026 operator evidence confirms `projectlocal.app` is verified and ready in Resend, `Project Local <notifications@projectlocal.app>` is the verified sender, a restricted Sending-access key scoped to the domain is stored only as `RESEND_API_KEY` in encrypted Vercel Production settings, the canonical notification base URL and sender are configured, and Resend open/click tracking are off. No credential value is recorded in this document or repository.

A direct Resend-dashboard email reached an approved Gmail inbox. This proves provider/domain/sender/basic inbox deliverability only. It did not invoke Project Local's Initial email action, access or mutate Supabase, claim or finalize `assignment_notification_deliveries`, issue an app-generated schedule credential, prove duplicate behavior, or exercise retry/failure paths.

The application transport was temporarily enabled only after provider readiness and no Project Local assignment email was sent. It was then removed, Vercel redeployed, and the resulting `https://projectlocal.app` deployment is Ready/Latest. `ASSIGNMENT_NOTIFICATION_EMAIL_TRANSPORT` and `ASSIGNMENT_NOTIFICATION_RECORDING_PATH` are currently absent, so application email is disabled. No production workspace, contact/grant, volunteer, Calendar item, assignment, notification-delivery row, or other real Bozeman product data was created.

The persistence/application gate remains blocking: after backup/recovery and provisioning prerequisites permit a reviewed controlled production-data test, Project Local must prove its own claim/provider/finalize round trip, duplicate protection, app-generated schedule access, retry/failure operations, and credential-free monitoring with real Resend. Launch remains `NO-GO`.
...
## 12.19 Calendar publication visibility

12.19 locally adds the first draft/private versus published/live visibility boundary for persisted Calendar items. New `create_calendar_item` calls derive both Follow-up Contact and `created_by_project_contact_id` from the authenticated project contact and store new items as `publication_state = 'draft'`. Draft rows have no published metadata; published rows require server-derived `published_at` and `published_by_project_contact_id`.

Calendar item reads still use the server-owned deterministic workspace/contact/capability context. The read model now returns only published rows plus the caller's own drafts. Unknown-owner legacy rows remain draft and fail closed rather than guessing ownership or publication state. Published items remain visible to authorized same-workspace project contacts under the existing strict coverage-bearing read rule.

Publishing uses `publish_calendar_item`, which requires effective `calendar.edit`, draft ownership, and same-workspace authorization. It is one-way/idempotent and does not send email, issue or reveal response links, expose volunteer schedules, activate public lookup, or depend on delivery success. Draft assignment preparation is limited to the draft creator with `assignments.edit`; response-token issuance/replacement/public reads/submits/reveal fail closed for drafts and remain available only through their existing reviewed boundaries after publication.

Application/product code still uses no service-role path and grants no broad direct table insert/update/delete to authenticated users. Local disposable validation (`npm run test:calendar-publication-visibility`) passed and proved draft owner-only reads, publish, published cross-contact visibility, draft assignment/token gating, direct table-write denial, grant lifecycle failure, role/title non-authorization, safe output, and zero-residue cleanup.

12.19.1 completed hosted non-production validation for migration `20260714121900_calendar_publication_visibility.sql`, `publish_calendar_item`, the replaced Calendar/assignment/response functions, and generated public-schema type parity against `project-local-staging` (`kfuujcfxoayukywvtaeh`). The target was `ACTIVE_HEALTHY`; staging advanced from `20260714121800` to `20260714121900` during the first migration application after a safe unapplied migration comment typo fix, and the final successful gate verified `20260714121900` before and after behavior validation. Hosted proof covered publication defaults, creator-only draft reads, published same-workspace cross-contact reads, publish authorization/idempotency, draft assignment/token/public-response gating, direct table-write denial, capability/contact/workspace/grant lifecycle isolation, role/title non-authorization, malformed input rejection, no email/delivery/public lookup/volunteer schedule/remembered-device coupling, safe output, generated-type parity after a UTF-8-safe refresh, and exact-run plus namespace zero residue. Product/runtime code did not change during the hosted gate.

## 12.20 Secure account-light volunteer schedule access

12.20 locally adds a dedicated account-light volunteer schedule access boundary. It does not reuse `assignment_response_tokens` and does not activate response-link reveal/copy. The new `volunteer_schedule_access_tokens` table stores only SHA-256 verifier hashes for opaque 256-bit bearers, with purpose/version constraints, server-derived workspace/volunteer/issuer scope, default 30-day expiry, 1-hour minimum, 90-day maximum, revocation, and last-used tracking.

Issuance and revocation use authenticated RPCs and require the caller to resolve to an active project contact with an effective same-workspace grant containing `assignments.edit`. Browser-provided workspace ids, contact ids, capability arrays, role/title strings, source/provenance fields, and protected token values are not trusted. Revoked, expired, inactive, wrong-contact, wrong-workspace, inactive-contact, inactive-workspace, and inactive/on-hold volunteer contexts fail closed.

The public read RPC validates only the dedicated volunteer schedule bearer and returns a safe schedule projection for that token's volunteer profile. It exposes only active assignments on active published Calendar items in the same workspace, including safe task/schedule/response/coverage fields and 12.21 response action eligibility. Draft/private items, canceled/archived items, other volunteers, other workspaces, response-token rows, token hashes, raw grants/capabilities, Auth metadata, SQL/RPC/provider details, and private volunteer/contact fields are not returned.

`/v/access/[token]` validates the bearer server-side, sets a session-only HttpOnly `pl-volunteer-schedule` cookie scoped to `/v`, and lands on a clean `/v/schedule` URL through a safe linked handoff. The cookie is SameSite=Lax, Secure on HTTPS, loopback-compatible locally, and is not stored in localStorage/sessionStorage or exposed to browser JavaScript. In 12.21, `/v/schedule` can submit Confirm, Can’t make it with optional notes, eligible response changes, and Confirm All through public RPCs that re-derive scope from the schedule credential and enforce the 48-hour/start-lock policy in the database. These schedule-cookie mutations use the distinct `volunteer_schedule` response source and do not create or consume assignment-response-token rows; `/respond/[token]` remains the `public_token` boundary. Email delivery, remembered devices, manual lookup, public volunteer self-editing, `/v/demo` cutover, and response-link activation remain future reviewed slices.

12.21.1 completed hosted non-production validation of this response boundary on `project-local-staging` (`kfuujcfxoayukywvtaeh`). The gate verified `ACTIVE_HEALTHY` target status, migration `20260714122100` before/after, generated-type parity, Confirm/Deny, denial notes, Confirm All, cutoff/start locks, response provenance separation, token separation, admin Calendar response truth, direct-table denial, route/cookie/mobile behavior, safe output, and exact-run plus namespace zero residue.

Local validation passed through `npm run test:volunteer-schedule-access` and `npm run test:volunteer-schedule-access:browser`, proving hash-only issuance, TTL bounds, publication-only reads, ready/empty/unavailable states, revocation, cookie security, no mock leakage, no unsafe output, direct table denial, isolation, and zero residue.

12.20.1 completed hosted non-production validation for migration `20260714122000_volunteer_schedule_access.sql`, `issue_volunteer_schedule_access`, `revoke_volunteer_schedule_access`, `read_volunteer_schedule`, hosted generated public-schema type parity, and hosted-backed route/cookie behavior against `project-local-staging` (`kfuujcfxoayukywvtaeh`). The target was `ACTIVE_HEALTHY`; staging was verified at `20260714122000` before and after the successful run. Hosted proof covered dedicated token separation from assignment-response tokens, hash-only storage, TTL policy, server-derived issuer/workspace/volunteer scope, issuance/revocation authorization through effective `assignments.edit`, public ready/ready-empty/unavailable reads, published-assignment filtering, current-response truth, safe projection, direct table denial, cross-contact/cross-workspace/grant lifecycle isolation, role/title non-authorization, inactive/on-hold volunteer failure, malformed/protected input rejection, multiple credential independence, last-used behavior, clean bearer exchange, HttpOnly SameSite=Lax session cookie, request host preservation, Not-you clearing, mobile layout, focus behavior, no email/public lookup/remembered-device/Confirm-Deny/response-link activation side effects, safe output, generated-type parity after a UTF-8-safe refresh, and exact-run plus namespace zero residue. Product/runtime code did not change during the hosted gate.
...
- **12.32 Production Observability Foundation — completed locally:** a server-only allowlisted event boundary now records representative Auth, persisted mutation, volunteer schedule/response, and assignment-email lifecycle outcomes without changing Auth, RLS, capabilities, workspace isolation, cookies/tokens, persistence, or user-facing errors. It rejects arbitrary payloads and raw errors, accepts only optional UUID correlation, and adds no service-role, database log table, browser telemetry, external sink, or production/staging access. The route-unused stale-`sending` detector accepts only an injected minimal delivery projection and cannot query, retry, send, or mutate. Application instrumentation is proven; operator production visibility/alerting remains blocking under [`PRODUCTION_OBSERVABILITY.md`](./PRODUCTION_OBSERVABILITY.md), and launch remains `NO-GO`.
