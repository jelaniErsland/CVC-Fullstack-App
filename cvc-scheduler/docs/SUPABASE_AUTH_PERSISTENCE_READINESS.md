...
This slice adds migration `20260714121700_calendar_source_selection.sql`, generated type updates, and `npm run test:calendar-source-selection`. Local disposable validation is green, but hosted non-production validation is still required before relying on the 12.17 DB/RPC boundary for beta use. Recommended next gate: `12.17.1 Hosted Staging Calendar Source Selection Validation Gate`.
