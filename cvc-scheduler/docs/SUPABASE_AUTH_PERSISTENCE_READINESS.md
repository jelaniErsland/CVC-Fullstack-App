...
The public read RPC validates only the dedicated volunteer schedule bearer and returns a safe read-only schedule projection for that token's volunteer profile. It exposes only active assignments on active published Calendar items in the same workspace, including safe task/schedule/response/coverage fields. Draft/private items, canceled/archived items, other volunteers, other workspaces, response-token rows, token hashes, raw grants/capabilities, Auth metadata, SQL/RPC/provider details, and private volunteer/contact fields are not returned.

`/v/access/[token]` validates the bearer server-side, sets a session-only HttpOnly `pl-volunteer-schedule` cookie scoped to `/v`, and lands on a clean `/v/schedule` URL through a safe linked handoff. The cookie is SameSite=Lax, Secure on HTTPS, loopback-compatible locally, and is not stored in localStorage/sessionStorage or exposed to browser JavaScript. `/v/schedule` is read-only; Confirm/Deny, Confirm All, email delivery, remembered devices, manual lookup, public volunteer self-editing, `/v/demo` cutover, and response-link activation remain future reviewed slices.
