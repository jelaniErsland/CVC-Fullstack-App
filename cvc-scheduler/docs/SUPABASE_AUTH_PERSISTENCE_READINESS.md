...
The runbook distinguishes application rollback from database rollback, records commit `082c960` as the currently verified production deployment baseline, preserves the migration-forward recovery policy, documents operational pause through grant revocation/expiration plus disabled email and Belgrade fallback, and defines safe post-recovery verification. 12.28.1 records operator dashboard evidence that production Supabase is on the Free plan, Supabase-managed backups are unavailable, no backup timestamp/status or retention exists yet, Supabase-managed restore to new project requires Pro plus physical backups, and PITR is unavailable but intentionally not required for the initial beta. The preferred current strategy is a tested encrypted independent logical backup system; Supabase Pro remains optional. 12.29 adds the Windows-first automation foundation and setup guide, but no production backup has run and no restore has passed. At least one reviewed backup path and restore test must pass before real Bozeman data is provisioned. Incident ownership remains operator-required. Launch remains `NO-GO`.
