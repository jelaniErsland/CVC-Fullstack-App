...
The runbook distinguishes application rollback from database rollback, records commit `082c960` as the currently verified production deployment baseline, preserves the migration-forward recovery policy, documents operational pause through grant revocation/expiration plus disabled email and Belgrade fallback, and defines safe post-recovery verification. Actual Supabase backup availability, retention, point-in-time recovery availability, restore initiation behavior, restore ownership, restore-test evidence, and incident ownership remain operator-required. Launch remains `NO-GO`.
