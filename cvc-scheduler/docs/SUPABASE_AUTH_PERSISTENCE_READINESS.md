...
Manual operator evidence, recorded in `docs/PRODUCTION_DEPLOYMENT_STATUS.md`, confirms that an existing approved Auth email could sign in by magic link, return through the final-domain production callback, open the admin shell, and fail closed on `/admin/calendar` and `/admin/volunteers` because no Project Local workspace/contact/grant exists yet. Launch remains `NO-GO` until production email, backup/restore, observability, operator provisioning, UI approval, and pilot evidence pass.
