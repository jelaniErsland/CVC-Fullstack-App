...
| Backup verification | Incomplete: production Supabase is on Free; Scheduled backups page states "Free Plan does not include project backups"; no Supabase-managed production backup timestamp/status or retention exists yet; preferred independent encrypted backup path is not implemented; Supabase Pro managed backups remain optional |
...
Backup, restore, and rollback readiness is documented in [`PRODUCTION_BACKUP_RECOVERY_RUNBOOK.md`](./PRODUCTION_BACKUP_RECOVERY_RUNBOOK.md). 12.28.1 operator dashboard evidence confirms production Supabase is on the Free plan; Supabase-managed scheduled backups are unavailable; no Supabase-managed backup timestamp/status or retention exists yet; PITR is unavailable and intentionally not required for the initial Bozeman beta; and Supabase-managed restore to new project requires Pro plus physical backups. This does not require a Pro upgrade before launch. Before real Bozeman data, Project Local must prove either the preferred independent encrypted logical backup path or the optional Supabase-managed Pro path, including backup timestamp/status/retention evidence and restore testing.
