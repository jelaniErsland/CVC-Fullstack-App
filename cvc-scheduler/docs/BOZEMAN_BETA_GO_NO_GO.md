...
| Production environment readiness | Partial | Production deployment/domain/Auth, provider/domain/sender configuration, initial-beta observability, complete backup/recovery, and the 12.36.5 established-production migration are proven. Production and the enabled/Ready permanent backup task now both use terminal migration `20260812123430` | Provision real Bozeman access, record the first authorized Notification Health execution, prove application-driven email, and complete the controlled pilot | Yes |
...
1. Provision real Bozeman workspace/contact/grants through the 12.14 operator boundary; the completed production migration must not be rerun.
2. As part of a separately reviewed controlled test/pilot, enable `ASSIGNMENT_NOTIFICATION_EMAIL_TRANSPORT=resend` only long enough to prove the Project Local Initial email action, production delivery-ledger claim/provider/finalize flow, duplicate behavior, schedule-access link, retry/failure procedure, and first production Notification Health execution with an approved recipient; disable it again if the reviewed step does not authorize continued sending.
