...
| Production environment readiness | Partial | Production deployment/domain/Auth is proven; production Supabase remains at `20260714122230`; provider/domain/sender configuration, initial-beta observability, the first independent encrypted backup, and full independent technical recovery are proven. 12.34.2 attributes the apparent grant blocker to restore interaction, and 12.34.3 proves deterministic source ACL reconstruction plus recovery-forward through `20260812123430` | Prove recurring backup scheduling/failure notification, record recovery/rollback ownership, separately review production migrations/Notification Health execution, prove application-driven email, provision real Bozeman, and complete the pilot | Yes |
...
| Backup/recovery | Technical recovery proven; operations required | 12.34 produced the unchanged six-file age-encrypted production backup at `2026-08-12T17:26:46.3144615Z`; the 62409-byte artifact and SHA-256/status/retention checks passed with no plaintext residue. 12.34.1 proved managed-role compatibility; 12.34.2 attributed the 26 grants to `RESTORE_INTERACTION`; 12.34.3 reconstructed the exact source ACL and completed local recovery-forward through 25 migrations at `20260812123430`, including generated types, RLS/FORCE RLS, exact direct/default privileges, product/application checks, and zero residue | Preserve the proven recovery path, prove recurring scheduling/failure notification, and record recovery/rollback owners. Supabase Auth platform configuration and Storage object BLOB recovery remain outside the logical package | Yes |
...
1. Before real Bozeman data, preserve the proven 12.34 encrypted backup, 12.34.1 managed-role rule, and 12.34.3 deterministic source ACL recovery; prove recurring scheduling/failure notification and record recovery/rollback owners using [`PRODUCTION_BACKUP_RECOVERY_RUNBOOK.md`](./PRODUCTION_BACKUP_RECOVERY_RUNBOOK.md).
