...
| Application observability foundation | Proven | 12.32 adds a server-only bounded event schema, representative beta-critical instrumentation, credential-free assignment-email stage/sent outcomes, a route-unused non-mutating stale-`sending` detector, and deterministic privacy/failure-isolation regression proof | Preserve the event/privacy contract while completing operator production visibility | No |
| Operator observability and backup/recovery | Configuration required | [`PRODUCTION_OBSERVABILITY.md`](./PRODUCTION_OBSERVABILITY.md) distinguishes proven application instrumentation from unproven runtime visibility/alerting; [`PRODUCTION_BACKUP_RECOVERY_RUNBOOK.md`](./PRODUCTION_BACKUP_RECOVERY_RUNBOOK.md) documents rollback, migration-forward policy, operational pause, recovery checks, 12.28.1 Free-plan backup evidence, preferred independent encrypted backups, and optional Supabase-managed Pro backups; 12.29 adds the independent backup automation foundation | Prove runtime/deployment event visibility, alert/incident ownership, conditions, stale-check read path/cadence, and one controlled safe event; complete operator backup setup, first encrypted backup, retention/status/notification, restore test, restore/rollback owners, and Belgrade fallback | Yes |
...
2. Configure and prove operator deployment/runtime event visibility, email failure alerts, incident ownership, and the authorized stale-delivery read path/check cadence; the 12.32 application instrumentation alone does not clear this gate.
