...
12.36 leaves that live task unchanged and still locked to `20260714122230`. The backup script and task registration boundary now allow only the reviewed current/next terminal migrations and expose one exact fail-closed update action from `20260714122230` to `20260812123430`; wrong-current, wrong-target, duplicate, enabled-task, and running-task states refuse. In a separately authorized production migration window, verify a recent artifact, disable the task, run the established-production migration/postflight, update only this lock, compare all other task metadata, and re-enable without a manual start. Until that sequence succeeds, do not advance the task lock; after production advances, do not leave the old lock in place because the next scheduled backup would intentionally fail its migration preflight.
