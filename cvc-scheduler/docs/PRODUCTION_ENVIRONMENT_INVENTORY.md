...
Production backup/recovery requirements are recorded in [`PRODUCTION_BACKUP_RECOVERY_RUNBOOK.md`](./PRODUCTION_BACKUP_RECOVERY_RUNBOOK.md). 12.28.1 confirms production Supabase Free-plan managed-backup limitations; PITR remains unnecessary for the initial beta. 12.29 adds the Windows-first independent foundation, 12.34 proves its first read-only age-encrypted production backup/checksum/status/retention behavior, and 12.34.1 through 12.34.3 prove managed-role compatibility, restore-interaction attribution, exact source ACL reconstruction, and complete local recovery-forward through `20260812123430`. 12.35 proves daily `03:15`/`StartWhenAvailable` task registration, human-visible safe failure notification, and recovery/rollback ownership. 12.35.11 proves the successful controlled scheduled-host production backup/checksum/retention path, and 12.35.12 enables the permanent task after harmless no-catch-up proof. Backup/recovery is complete and non-blocking. Backup artifacts must never enter the public repository, and the logical database package does not prove Supabase Auth platform configuration or Storage object BLOB recovery.
...
Do not rerun the bootstrap gate as the established-production migration check after approved Auth identities or real product data exist. 12.36 locally proved `npm run test:production-established-schema`; 12.36.5 then completed the separately authorized no-fixture production preflight/apply/postflight through exactly `20260811123300` and `20260812123430`. Production and the permanent backup task now both terminate/expect `20260812123430`. Do not rerun that completed migration or move the task lock backward. After deployment, Auth, or origin changes, rerun the production deployment smoke gate:
