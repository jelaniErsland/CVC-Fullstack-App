...
This inventory began in Iteration 12.24. Iteration 12.31 selects Resend and adds its server-only application adapter without configuring provider infrastructure, changing DNS/Vercel, creating real data, sending email, or committing secrets.
...
| `ASSIGNMENT_NOTIFICATION_EMAIL_TRANSPORT` | Required for email sending; disabled otherwise | Server-only | `recording` for local QA; `resend` only with fake-network adapter regression | `recording` only for hosted QA unless separately reviewed | Empty/disabled until Resend activation; then exact value `resend` | App/email provider | Non-secret enum: `recording` or `resend` | Yes | Focused adapter regression and controlled provider test | Unknown/incomplete transport fails closed; no fallback | Placeholder/default only |
...
| `ASSIGNMENT_NOTIFICATION_FROM` | Required only when notification transport active | Server-only | Synthetic sender for recording QA | Synthetic sender for recording QA | Exact Resend-verified sender identity | Email provider/operator | Sender identity, not a secret | Yes | Configuration check plus controlled deliverability test | Send boundary unavailable | Placeholder only |
...
- `ASSIGNMENT_NOTIFICATION_EMAIL_TRANSPORT` remains empty/disabled until Resend domain/sender/key/monitoring/test-recipient activation is approved; it becomes exactly `resend` only for the controlled deliverability step.
...
4. Verify production email remains disabled until every Resend activation requirement in the production deployment runbook is complete.
