...
`npm run test:assignment-notification-email:resend` uses fake HTTP responses and does not contact Resend. `npm run test:assignment-notification-email` continues to prove the local disposable database behavior. No Calendar UI, schema, migration, RPC, RLS, Auth, schedule credential/cookie, publication, assignment, response, hosted data, or external configuration changed in 12.31.
