...
Local validation: `npm run test:calendar-source-selection` passes against disposable local fixtures and proves persisted selector behavior, preset-backed create/read/edit, one-off continuity, fake/archived/wrong-workspace preset rejection, view-only/wrong-contact/role-only denial, direct table-write denial, and zero residue. The Calendar browser regression was extended to prove the real preset selector plus preset-backed create/reload/edit/reload. Because this slice adds migration `20260714121700_calendar_source_selection.sql`, RPC behavior, and generated-type changes, hosted non-production validation remains required as `12.17.1` before beta reliance.
