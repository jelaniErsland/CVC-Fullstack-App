...
12.17 locally extends the Calendar write boundary from one-off timed items only to timed items sourced either from an active persisted task preset or from a custom one-off definition. This is not a `/admin/tasks` cutover: the task-preset selector is Calendar-owned, server-only, explicit-selector based, same-workspace scoped, and requires `tasks.view` in the already-resolved deterministic Calendar route context.

The route now passes a safe `taskPresetSelector` state to the Calendar client. Ready selector states carry active persisted presets with safe fields only: preset id, workspace-derived project reference, name, description, high-level type/category, default needed count, volunteer visibility, bounded custom-field definitions, and safe system identity. Missing `tasks.view` makes preset selection unavailable but does not block custom one-off timed creation for a contact with `calendar.edit`. No mock task presets are used as Calendar creation truth.

Create continues through `create_calendar_item`; preset-backed creation supplies a real same-workspace active `task_preset_id`, and one-off creation supplies a bounded title/type. 12.17 adds `update_calendar_item_preset_timed`, which updates only preset-backed timed occurrence date, start time, end time, needed count, notes, and custom values. It rejects one-off rows, archived/canceled rows, cross-workspace actors, missing `calendar.edit`, invalid time ordering, invalid needed counts, and unsupported source changes. One-off edits continue through `update_calendar_item_one_off_timed`.

The read model now keeps `title_snapshot` as the Calendar display label for scheduled occurrences while carrying task preset id/label/type as safe source metadata. This prevents a reusable preset name from replacing occurrence-specific scheduled-item labels in the Calendar shell.

Local validation: `npm run test:calendar-source-selection` passes against disposable local fixtures and proves persisted selector behavior, preset-backed create/read/edit, one-off continuity, fake/archived/wrong-workspace preset rejection, view-only/wrong-contact/role-only denial, direct table-write denial, and zero residue. The Calendar browser regression was extended to prove the real preset selector plus preset-backed create/reload/edit/reload.

12.17.1 completed hosted non-production validation on `project-local-staging` (`kfuujcfxoayukywvtaeh`) with the project `ACTIVE_HEALTHY`. Staging was already at migration `20260714121700` when the successful gate ran and remained at `20260714121700` afterward. Hosted generated public-schema types matched the committed types. The gate validated selector authorization and safe projection, same-workspace scoping, active-only lifecycle eligibility, missing-`tasks.view` selector unavailability while custom one-off creation still works, role/title non-authorization, preset-backed timed create/edit, source snapshot semantics, fake/wrong-workspace/archived preset rejection, mixed-source rejection, source immutability and no one-off/preset conversion, mutation capability enforcement, cross-contact/cross-workspace isolation, revoked/expired/inactive grant denial, direct table-write denial for `calendar_items` and `task_presets`, read-model compatibility, existing one-off compatibility, existing preset-backed row compatibility, malformed input rejection, safe output, exact-run cleanup, namespace zero residue, and hosted disposable residue count `0`.

## 12.18 Calendar Volunteer Assignment Picker and Create/Cancel

12.18 locally activates the first persisted assignment management path from `/admin/calendar` without changing publication, volunteer-visible schedule, response-link, or delivery behavior. The Calendar inspector now displays active persisted assignments and current response states from assignment/current-response truth. It offers a searchable ready-volunteer picker only when the resolved contact has `volunteers.view`; assignment create/cancel controls require `assignments.edit`.

The new server-only picker helper reads explicit `volunteer_profiles`, `calendar_assignments`, and `assignment_responses` columns. It projects only volunteer profile id, display name, congregation, assignment id, Calendar item id, volunteer profile id, and response status. It does not project email, phone, profile notes, questionnaire data, response tokens, URLs, raw grants/capabilities, SQL/RPC detail, or provider errors.

12.18 adds migration `20260714121800_calendar_assignment_management.sql` with authenticated RPC `create_calendar_assignments_batch`. The RPC atomically assigns `1..25` active ready same-workspace volunteers to one active timed/date-based Calendar item, requires effective `assignments.edit`, rejects duplicates and wrong-workspace/ineligible volunteers, and creates `needs_response` current response rows. Cancel continues through `cancel_calendar_assignment` and marks active assignments `canceled`.

Assignment-derived coverage remains based on `calendar_assignments` and current `assignment_responses`; no Calendar item counters, mock `filledCount`, assigned-volunteer arrays, response links, publication state, email, delivery, or assignment-detail entry links were added. Local validation uses `npm run test:calendar-assignment-management` and proves persisted picker projection, atomic create, cancel, over-target coverage, capability/workspace/contact isolation, direct table-write denial, no response-token creation, and zero-residue cleanup.

## 12.18.1 Hosted Staging Assignment Management Validation

12.18.1 completes the hosted non-production validation gate for the 12.18 assignment-management boundary against `project-local-staging` (`kfuujcfxoayukywvtaeh`). Staging is validated at migration `20260714121800`; the first gate run advanced the reviewed `20260714121800_calendar_assignment_management.sql` migration, and the final successful gate verified that migration level before and after behavioral validation.

The hosted gate compares generated public-schema types to `lib/supabase/database.types.ts`; one narrow generated-type parity correction was committed for `create_calendar_assignments_batch.p_assignment_note`, matching hosted generation as `string`. Product/runtime code did not change during the gate.
...
12.20 locally adds the first volunteer-facing schedule read path over the existing Calendar assignment truth. It does not change `/admin/calendar` read/write semantics, publication truth, assignment mutation behavior, or response-link activation. It adds a dedicated hash-only `volunteer_schedule_access_tokens` credential table and narrow RPCs for issuing, revoking, and reading a volunteer schedule.
...
The route continues to use the dedicated HttpOnly volunteer schedule cookie; the browser never receives the bearer. Schedule-cookie responses record `response_source = 'volunteer_schedule'`, while the separate assignment-response-token route continues to record `public_token`. Assignment rows plus the one current `assignment_responses` row remain the response and coverage truth. `/v/demo`, public lookup, remembered devices, assignment-detail entry links, response-link reveal/copy, email, Communications, and real Bozeman data remain out of scope. Local database/browser validation and 12.21.1 hosted staging validation passed. The hosted gate validated `project-local-staging` (`kfuujcfxoayukywvtaeh`) as `ACTIVE_HEALTHY` at migration `20260714122100`, generated-type parity, Confirm/Deny, denial notes, Confirm All, 48-hour/start locks, response-source provenance separation, admin Calendar response truth, direct table denial, hosted route/cookie/mobile proof, safe output, and exact-run plus namespace zero residue.

## 12.22 Initial Assignment Notification Email Boundary

12.22 locally adds the first initial assignment notification email boundary without changing Calendar publication truth, assignment truth, volunteer response truth, or response-link reveal/copy policy. The action is explicit from the `/admin/calendar` inspector and never runs from render, page load, GET, publish, assignment creation, prefetch, hover/focus, or client effects.

Notification eligibility is re-derived from persisted state at send time. The item must be active, published, same-workspace, and future/not-started; assignments must be active; volunteers must be active/ready with valid recipient email; a successful initial delivery for the same assignment/kind/template version must not already exist; and the Calendar item's Follow-up Contact must have reviewed volunteer-facing display/email data. Missing recipient or Follow-up Contact data is represented as safe skipped status, not as a mock fallback or raw provider/database error.

Migration `20260714122200_initial_assignment_notifications.sql` adds explicit volunteer-facing Follow-up Contact fields to `project_contacts`, adds the `assignment_notification_deliveries` ledger, and adds authenticated RPCs for notification summaries, send claiming, and delivery finalization. The ledger records only bounded operational metadata such as delivery state, template version, recipient email snapshot, provider message id, safe failure code, and initiating project contact. It does not persist the schedule bearer, verifier, full URL, response URL, raw provider dump, SQL, stack trace, credentials, raw grants, or capability arrays.

The send path uses the existing dedicated volunteer schedule access boundary. The helper issues a hash-only `/v/access/[token]` credential, builds the full URL only in memory for the provider, and revokes the token after a provider failure where safely possible. Successful delivery is terminal for that assignment/template version; failed attempts are retryable; stale `sending` claims expire for recovery.

The provider boundary is server-only and disabled by default. The local implementation includes a deterministic recording transport for QA that writes redacted delivery summaries and never stores or prints the bearer/full URL/raw recipient/provider payload. No real provider integration, automatic reminders, Communications persistence, public lookup, remembered devices, response-link reveal/copy, assignment-detail entry links, real Bozeman data, service-role application path, or Calendar counter behavior was added.

Local validation uses `npm run test:assignment-notification-email` and proves claim/finalize behavior, duplicate prevention, missing-recipient/missing-Follow-up-Contact skips, explicit route action wiring, schedule-access issuance, redacted recording output, direct table denial, capability/contact/workspace isolation, disabled transport behavior, and zero disposable residue.

12.22.1 completes the required hosted non-production gate for this boundary against `project-local-staging` (`kfuujcfxoayukywvtaeh`). Hosted validation is now complete through migration `20260714122230`: the original `20260714122200` migration was applied, then hosted-discovered malformed-recipient and finalize-bound defects were corrected with forward migrations `20260714122210`, `20260714122220`, and `20260714122230`. The hosted gate verified generated public-schema type parity, notification summaries, claim/finalize behavior, duplicate/concurrency/retry/stale recovery, malformed recipient handling, safe provider-message-id bounds, schedule-access secrecy, no response-token or response-link reveal activation, hosted browser explicit-send and retry behavior using the recording transport only, safe output, and exact-run plus namespace zero residue.

## 12.23 Bozeman Beta UI Polish and Launch Gate

12.23 does not change the Calendar storage model, RPC contract, generated types, or migration state. It polishes the existing persisted Calendar scheduling loop so the inspector reads as an operational scheduling surface rather than a backend diagnostic: assignment rows retain assignment/current-response truth, the picker keeps the reviewed safe volunteer projection, draft/publish copy keeps email separate from visibility, and the Initial email card now emphasizes `Notify volunteers` while preserving ready/already-sent/missing-email/missing-Follow-up-Contact distinctions.

The launch gate remains separate from Calendar persistence. `lib/readiness/bozemanBetaLaunchGate.server.ts` records a server-only, route-unused NO-GO launch decision until production provider/domain/deployment/Auth redirect/observability/backup/operator/pilot requirements are evidenced. It cannot create Calendar rows, send email, mutate deployment configuration, use a service-role application path, or activate response-link reveal/copy. `npm run test:bozeman-beta-launch-gate` protects that boundary, and `npm run test:bozeman-beta-launch:hosted` verifies the approved staging target remains `ACTIVE_HEALTHY` at migration `20260714122230` with generated-type parity.

12.23.1 adds `npm run test:bozeman-beta-e2e:hosted` as the integrated hosted staging beta-loop proof, locked to `project-local-staging` (`kfuujcfxoayukywvtaeh`) by `RUN_HOSTED_BOZEMAN_BETA_E2E_VALIDATION=project-local-staging:kfuujcfxoayukywvtaeh`. The gate validated the target as `ACTIVE_HEALTHY`, migration `20260714122230` before and after, and generated-type parity, then used one disposable namespace through the real product routes: persisted Volunteers Add/Edit, Calendar one-off create/edit, preset-backed creation, assignment picker/create, draft privacy, publication, recording-only Initial email, duplicate-send prevention, secure schedule handoff, Confirm, `Can’t make it`, Confirm All, admin response/coverage truth, view-only/wrong-workspace denial, missing-prerequisite handling, inside-48-hour guidance, started-state lock, invalid credential handling, desktop/390px schedule proof, safe-output checks, screenshot capture/removal, and exact-run/namespace/Auth residue counts of `0`. It adds no Calendar schema/RPC change, product runtime feature, real provider send, response-link reveal/copy activation, public lookup, remembered-device behavior, or production access.

## 12.31 Resend initial-assignment transport

12.31 selects Resend and adds a production-capable direct HTTPS adapter inside the existing 12.22 server-only boundary. Disabled and recording modes remain unchanged; `resend` requires an HTTPS origin, sender address, and server-only `RESEND_API_KEY` and fails closed without fallback. The existing database ledger remains authoritative for claim/finalize, duplicate prevention, failure retry, and stale recovery; Resend receives a deterministic non-secret idempotency header as defense in depth. Only bounded provider message ids may reach the ledger, while provider/network failures remain `provider_send_failed` and raw responses, credentials, bearers, and full schedule URLs never enter safe output.

`npm run test:assignment-notification-email:resend` uses fake HTTP responses and does not contact Resend. `npm run test:assignment-notification-email` continues to prove the local disposable database behavior. No Calendar UI, schema, migration, RPC, RLS, Auth, schedule credential/cookie, publication, assignment, response, hosted data, or external configuration changed. Real sender/domain/key configuration, monitoring, and deliverability proof remain launch blockers.
