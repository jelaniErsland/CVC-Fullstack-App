...
20. `12.25 Production Supabase Migration, Type-Parity, and Read-Only Validation Gate`
   - Prepared as the first exact-target production schema gate for `project-local-production` (`wdlaauzknfggoqldolmx`) and terminal migration `20260714122230`. The actual production migration/type-parity/read-only validation remains pending until this 12.25 gate is committed and rerun from a clean worktree with the exact opt-in. The gate does not create fixtures, Auth users, product rows, storage objects, real Bozeman data, email, deployment, DNS changes, Auth redirect changes, service-role application behavior, response-link reveal/copy activation, or launch approval.
