...
21. `12.26 Production Deployment, Auth, and Read-Only Smoke Validation`
   - Completed as public smoke/readiness evidence and gate preparation only. Vercel project `project-local` is live at `https://project-local-one.vercel.app`, production Auth is enforced, Supabase Auth Site URL/callback are configured for that temporary origin, manual magic-link sign-in succeeded, a signed-in user without Project Local workspace/contact/grants failed closed on persisted admin routes, and `npm run test:production-deployment-smoke` is added for exact-origin public landing/admin/volunteer unavailable checks with no mutation. Because the command is new and refuses dirty worktrees, rerun it after commit; the same public HTTP assertions passed during implementation through a separate non-mutating diagnostic. Launch remains `NO-GO`; final custom domain, email, backup/restore, observability, operator provisioning, UI approval, and pilot evidence remain blocking.
22. `12.27 Final Production Domain and Auth Validation`
   - Completed as final-domain readiness evidence and smoke-gate retargeting only. Canonical production origin `https://projectlocal.app` is manually configured and working, Vercel project `project-local` remains live with temporary fallback alias `https://project-local-one.vercel.app`, production Auth is enforced, Supabase Auth Site URL/callback are configured for the final domain, the temporary Vercel callback remains allowlisted, manual magic-link sign-in returned through the final domain, a signed-in user without Project Local workspace/contact/grants failed closed on persisted admin routes, and `npm run test:production-deployment-smoke` now targets `https://projectlocal.app`. Because the command refuses dirty worktrees, rerun it after commit; the same public HTTP assertions passed during implementation through a separate non-mutating diagnostic. Launch remains `NO-GO`; exact clean-tree smoke, email, backup/restore, observability, operator provisioning, UI approval, and pilot evidence remain blocking.
