...
- 12.16 Calendar Create/Edit Scheduled Item Implementation. Completed locally as the first permanent `/admin/calendar` scheduled-item write path. Authorized project contacts with effective `calendar.edit` can create and edit persisted one-off timed Calendar items from the existing Calendar UI; read-only contacts can view but cannot mutate. The slice adds migration `20260714121600_calendar_item_management.sql`, nullable `follow_up_project_contact_id`, `0..99` timed/date-based needed counts, a create RPC that derives Follow-up Contact from the authenticated scheduler, and an allowlisted `update_calendar_item_one_off_timed` RPC. It preserves server-owned workspace/contact/capability derivation, no mock Calendar fallback/mixing, assignment-derived coverage truth, and direct table-write denial. Task-preset selector, all-day/date-based authoring, archive/delete, publication, assignment picker/mutations, delivery, public lookup, remembered devices, response-link activation, and production data remain out of scope. Because schema/RPC/generated-type behavior changed, the next slice must be `12.16.1 Hosted Staging Calendar Item Management Validation Gate` before 12.17.
