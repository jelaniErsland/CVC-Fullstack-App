...
11. `12.20 Secure Account-Light Volunteer Schedule Access`
   - Completed locally and hosted-validated through 12.20.1: dedicated hash-only volunteer schedule access credentials, `/v/access/[token]` bearer exchange, and read-only `/v/schedule` for a volunteer's own published assignments.
...
- 12.20 Secure Account-Light Volunteer Schedule Access. Completed locally as the first dedicated volunteer-facing schedule access path. Authorized contacts with effective `assignments.edit` can issue/revoke hash-only volunteer schedule access credentials; `/v/access/[token]` exchanges the bearer for a session-only HttpOnly cookie; and `/v/schedule` reads only that volunteer's published assignments. The slice adds migration `20260714122000_volunteer_schedule_access.sql`, generated public-schema type changes, dedicated RPCs, safe route/cookie handling, local DB validation, and local browser proof. Confirm/Deny, email, remembered devices, public lookup, `/v/demo` cutover, response-link reveal/copy activation, and real Bozeman data remain out of scope.
