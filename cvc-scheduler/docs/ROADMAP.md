...
3. `12.15.1 Hosted Staging Migration + Volunteer Profile Management Validation Gate`
   - Pending: the opt-in hosted gate exists and is locked to `project-local-staging` (`kfuujcfxoayukywvtaeh`), but the approved staging project is currently `INACTIVE`, so hosted DB/RLS/RPC validation has not passed yet.
...
- 12.15.1 Hosted Staging Migration + Volunteer Profile Management Validation Gate. Pending/blocked, not passed. Added `scripts/hosted-volunteer-profile-management-regression.mjs` and `npm run test:volunteer-profile-management:hosted` as an exact opt-in hosted gate for `project-local-staging` (`kfuujcfxoayukywvtaeh`). The gate verifies migration `20260714121500`, generated types, manual create/edit RPC behavior, questionnaire provenance compatibility, RLS/capability isolation, grant lifecycle, direct table-write denial, malformed/protected input rejection, and hosted disposable cleanup with zero residue. The approved staging project currently reports `INACTIVE`, so hosted DB login-role creation times out before migration/RPC validation can run. Next work is to restore the approved staging target and rerun 12.15.1; do not begin 12.16 until the hosted gate is green.
