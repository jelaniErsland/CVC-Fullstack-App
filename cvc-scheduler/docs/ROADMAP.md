...
3. `12.15.1 Hosted Staging Migration + Volunteer Profile Management Validation Gate`
   - Completed: non-production `project-local-staging` (`kfuujcfxoayukywvtaeh`) advanced from `20260705000000` to `20260714121500`, and the hosted gate passed generated-type, manual create/edit, questionnaire provenance, RLS/capability, isolation, direct-table-denial, safe-error, and zero-residue checks.
4. `12.16 Calendar Create/Edit Scheduled Item Implementation`
   - Completed: first narrow one-off timed Calendar item create/edit path.
...
9. `12.19 Draft/Private Versus Published/Live Calendar Visibility`
   - Completed locally and hosted-validated through 12.19.1: persisted Calendar items default to private drafts owned by the creator, publish through a one-way server boundary, and separate visibility from email/public response-link/volunteer schedule activation.
...
11. `12.20 Secure Account-Light Volunteer Schedule Access`
   - Completed locally and hosted-validated through 12.20.1: dedicated hash-only volunteer schedule access credentials, `/v/access/[token]` bearer exchange, and initial read-only `/v/schedule` for a volunteer's own published assignments.
12. `12.20.1 Hosted Staging Volunteer Schedule Access Validation Gate`
   - Completed: non-production `project-local-staging` (`kfuujcfxoayukywvtaeh`) is validated at `20260714122000`; generated-type parity, schedule-token issuance/revocation/read RPCs, hosted public route/cookie behavior, isolation, direct-table denial, and zero-residue cleanup passed.
13. `12.21 Volunteer Confirm/Deny Round Trip`
   - Completed locally and hosted-validated through 12.21.1: `/v/schedule` can persist Confirm, Can’t make it with optional notes, reversible eligible response changes, and Confirm All through server-owned credential-scoped RPCs; `/respond/[token]` now shares the same 48-hour/start-lock mutation policy.
...
18. `12.23.1 Hosted End-to-End Bozeman Beta Loop Integration Gate`
   - Completed against non-production `project-local-staging` (`kfuujcfxoayukywvtaeh`) at migration `20260714122230`: one disposable namespace exercised the real hosted product loop across Auth/session, Volunteers Add/Edit, Calendar one-off/preset scheduling, assignment, draft privacy, publication, recording-only Initial email, secure schedule handoff, Confirm/Deny/Confirm All, admin response truth, negative paths, safe output, generated-type parity, screenshot capture/removal, and zero hosted residue. Conclusion remains `NO-GO` until production launch blockers are resolved.
19. `12.24 Production Environment, Deployment, Domain, and Auth Readiness`
   - Completed as operator handoff/readiness only: Vercel is recommended as the first production host, production environment inventory/deployment runbook/Jelani checklist were added, and a server-only route-unused production readiness contract protects the `NO-GO` conclusion. No production Supabase project, deployment, DNS, real data, real email, service-role application behavior, response-link activation, or deferred feature was added.
20. `12.25 Production Supabase Migration, Type-Parity, and Read-Only Validation Gate`
   - Completed as the first exact-target initial/bootstrap empty-production schema gate for `project-local-production` (`wdlaauzknfggoqldolmx`). Production advanced from a clean initial migration state to `20260714122230`; generated public-schema type parity, public Supabase connectivity, empty product/Auth/storage counts before Auth setup, and structural RLS/security checks passed. No fixtures, Auth users, product rows, storage objects, real Bozeman data, email, deployment, DNS changes, Auth redirect changes, service-role application behavior, response-link reveal/copy activation, staging mutation, or launch approval were added. Preserve this historical bootstrap evidence; 12.36 supplies the separate established-production gate for later live-state migrations.
21. `12.26 Production Deployment, Auth, and Read-Only Smoke Validation`
   - Completed as public smoke/readiness evidence and gate preparation only. Vercel project `project-local` is live at `https://project-local-one.vercel.app`, production Auth is enforced, Supabase Auth Site URL/callback are configured for that temporary origin, manual magic-link sign-in succeeded, a signed-in user without Project Local workspace/contact/grants failed closed on persisted admin routes, and `npm run test:production-deployment-smoke` was added for exact-origin public landing/admin/volunteer unavailable checks with no mutation. This was superseded by the final-domain 12.27 smoke completion. Launch remains `NO-GO`; email, backup/restore, observability, operator provisioning, UI approval, and pilot evidence remain blocking.
22. `12.27 Final Production Domain and Auth Validation`
   - Completed as final-domain readiness evidence and public smoke validation. Canonical production origin `https://projectlocal.app` is manually configured and working, Vercel project `project-local` remains live with temporary fallback alias `https://project-local-one.vercel.app`, production Auth is enforced, Supabase Auth Site URL/callback are configured for the final domain, the temporary Vercel callback remains allowlisted, manual magic-link sign-in returned through the final domain, a signed-in user without Project Local workspace/contact/grants failed closed on persisted admin routes, commit `082c960` was pushed to `origin/master`, the Vercel Production deployment sourced from `082c960` reached Ready, and the exact final-domain production smoke passed before push and after deployment with exit code `0`. Launch remains `NO-GO`; email, backup/restore, observability, operator provisioning, UI approval, and pilot evidence remain blocking.
23. `12.28 Production Backup, Recovery, and Rollback Readiness`
   - Completed as route-unused readiness/documentation only. Added `lib/readiness/productionRecoveryReadiness.server.ts`, [`PRODUCTION_BACKUP_RECOVERY_RUNBOOK.md`](./PRODUCTION_BACKUP_RECOVERY_RUNBOOK.md), and `npm run test:production-recovery-readiness` to distinguish Vercel app rollback from database recovery, preserve migration-forward policy, document grant-revocation/email-disable operational pause, define safe post-recovery checks, and keep backup/retention/restore ownership/restore-test evidence blocking. 12.28.1 records operator evidence that production Supabase is on Free, Supabase-managed backups and restore-to-new-project are unavailable on that plan, PITR is unavailable but intentionally not required for the initial beta, independent encrypted backups are preferred, and Supabase Pro remains optional. No production access, restore, dump, data, email, migration, generated-type, or service-role behavior changed.
24. `12.29 Independent Encrypted Production Backup Automation Foundation`
   - Completed as Windows-first operator-owned foundation only. Added PowerShell scripts for DPAPI secret setup, guarded Supabase CLI logical dumps, age encryption, private OneDrive encrypted artifact output, checksum/status, bounded retention, explicit Task Scheduler registration, and local/disposable restore-drill guardrails. Added [`INDEPENDENT_PRODUCTION_BACKUP_SETUP.md`](./INDEPENDENT_PRODUCTION_BACKUP_SETUP.md) and `npm run test:production-independent-backup`. No production backup, restore, task registration, credential, key, data, migration, generated type, GitHub Actions backup artifact, or runtime behavior changed.
25. `12.30 First Beta UI Visual Integration Attempt`
   - Functionally validated but rejected in product-owner visual review. The persisted Calendar, Volunteers, and volunteer-schedule behavior remained intact, but the presentation stayed materially closer to prototype scaffolding than to the approved Project Local concepts. 12.30 is not an approved design baseline.
26. `12.30.1 Beta UI Visual Direction Correction`
   - Completed and explicitly product-owner approved by Jelani. The permanent references and contract are in [`design/approved-project-local-ui`](./design/approved-project-local-ui/); the shared shell, real Calendar, real Volunteers, and secure volunteer schedule use the approved brand, hierarchy, density, navigation, surfaces, and inspector patterns without changing schema, RPCs, authorization, tokens, email, persistence, production data, or feature scope. All six real-route desktop/390px captures in [`previews/beta-review`](./previews/beta-review/) were reviewed and accepted. The beta-critical UI gate is proven and no longer blocking; launch remains `NO-GO` because email/provider deliverability, observability, backup/restore, real Bozeman provisioning, and controlled pilot evidence remain open.
27. `12.31 Production Transactional Email Provider Integration`
   - Resend is selected and integrated behind the existing server-only 12.22 initial-assignment email boundary. The explicit disabled/recording/resend configuration fails closed; direct HTTPS requests carry concise persisted-truth-only Project Local text/HTML plus a deterministic non-secret provider idempotency key; only a bounded provider message id reaches the authoritative delivery ledger; and provider/network errors collapse to the existing safe failure code. Focused regression uses fake HTTP responses and preserves recording and database-ledger duplicate/retry proof. No real email, provider/DNS/Vercel configuration, schema/RPC/RLS/Auth/token/cookie change, hosted access, or UI change occurred. Launch remains `NO-GO` until Resend domain/sender/key configuration, deliverability/monitoring evidence, backup/restore, observability, real provisioning, and pilot gates pass.
28. `12.31.1 Resend Operator Configuration and Provider Deliverability Evidence`
   - August 10, 2026 operator evidence proves the verified `projectlocal.app` Resend sending domain, verified `Project Local <notifications@projectlocal.app>` sender, restricted domain-scoped production key stored only in encrypted Vercel Production settings, canonical base URL/sender values, disabled open/click tracking, and direct Resend-dashboard delivery to an approved Gmail inbox. This proves provider/domain/sender/basic inbox delivery only. No Project Local Initial email or production delivery-ledger flow ran, no real Bozeman product data was created, and the application transport is currently absent/disabled after a Ready/Latest redeployment. Application-driven delivery, duplicate/schedule-link/retry proof with real Resend, email-failure monitoring, backup/restore, provisioning, and pilot evidence remain blocking; launch remains `NO-GO`.
29. `12.32 Production Observability Foundation`
   - Adds a server-only allowlisted structured event seam for representative beta-critical Auth, Calendar, Volunteer, assignment, schedule-access, volunteer-response, and assignment-email outcomes. Assignment-email claim, schedule-access, provider, finalization, sent, and stale-`sending` events are safely distinct; callers cannot attach arbitrary payloads; logging failure cannot alter product behavior; and the route-unused stale detector neither queries nor mutates the ledger. Deterministic regressions prove the privacy boundary without external telemetry. The application foundation is proven, but production runtime/deployment visibility, alert/incident ownership, actionable conditions, stale-check cadence/read path, and one controlled observable-event proof remain blocking. See [`PRODUCTION_OBSERVABILITY.md`](./PRODUCTION_OBSERVABILITY.md). Launch remains `NO-GO`.
30. `12.32.1 Production Operator Observability Evidence`
   - Records August 11, 2026 proof that the 12.32 structured events reach searchable/filterable Vercel Production Logs, deployment/build status is reviewable in Vercel Deployments, the Project Local product/operator owner owns alert/incident response, and a controlled invalid-token request safely produced `schedule_access.exchange_failure` with no prohibited data. Runtime visibility, ownership, action conditions, deployment/build review, and one controlled observation are proven. The authorized stale-delivery production read path, cadence, threshold/escalation proof, and practical notification mechanism beyond manual Vercel Hobby review remain blocking. No application behavior, data, email state, external configuration, paid service, webhook, tracking, analytics, cron, or background job changed. Launch remains `NO-GO`.
31. `12.33 Authenticated Assignment Notification Health`
   - Adds the minimum authenticated, exactly-one-workspace, capability-gated RPC over the authoritative notification ledger; it returns only id/state/expiry for at most 100 `sending` rows ordered earliest-expiry first. A strict server-only reader reuses the existing stale detector and one safe event, while the unlinked dynamic/no-store Notification Health route shows only healthy, stale-count, or unavailable states. Disposable local and `project-local-staging` validation through `20260811123300` prove type parity, authorization, ambiguity denial, workspace isolation, minimal projection, bounds/order, stale/fresh behavior, direct table denial, no mutation, and zero residue. The documented after-batch/before-retry/end-of-active-day cadence and pause-on-repeat policy make manual notification sufficient for the initial tiny controlled beta; automated alerting is not required at that scale. Production execution and real production stale-row observation remain controlled-pilot evidence. Application email remains disabled and launch remains `NO-GO` for email, backup/restore, provisioning, and pilot blockers.
32. `12.34 Independent Production Backup Execution and Local Restore Proof`
   - One guarded read-only Session Pooler preflight proved exact production target/database and terminal migration `20260714122230`; exactly one backup execution produced the unchanged six-file package, encrypted it before private OneDrive persistence, and recorded `2026-08-12T17:26:46.3144615Z`, `62409` bytes, matching SHA-256, retention, and zero plaintext residue. The loopback disposable restore stopped at `roles.sql`; a synthetic local Supabase role dump reproduced the managed-role privilege failure while all required platform roles were already present. The package contract was not weakened, production was not mutated, and full restore/post-restore proof remains blocking. Launch remains `NO-GO`.
33. `12.34.1 Supabase Managed-Role Restore Compatibility and Full Local Restore Proof`
   - Reused the existing encrypted artifact and kept the exact six-file package. A deterministic parser classified all eight actual role statements, verified four represented platform roles/settings/parameter privilege rather than recreating them, preserved supported synthetic user-role material, and refused unsupported SQL. Exactly one fresh loopback restore passed roles, schema, all 23 migrations through `20260714122230`, data, baseline functions, pending-function absence, RLS, and FORCE RLS. Verification then failed closed on 26 direct `TRUNCATE` grants—`anon` and `authenticated` across all 13 Project Local tables—before type/product/application checks. Full recovery remains blocking; production/staging were untouched and launch remains `NO-GO`.
34. `12.34.2 Restored Grant Attribution`
   - Reproduced and classified the 26 direct `TRUNCATE` grants as `RESTORE_INTERACTION`, not historical source ACL truth. The production artifact and six-file contract remained unchanged; no grant was waived or normalized without source evidence.
35. `12.34.3 Deterministic Source ACL Recovery and Hosted Staging Privilege Hardening`
   - Parses the real encrypted artifact's historical table ACLs deterministically, reconstructs that exact source posture after restore, and completes local recovery-forward through `20260811123300` and `20260812123430`. Final local proof covers 25 migrations, type parity, all-table RLS, exact FORCE RLS, the centralized exact privilege allowlist, protected postgres future-table defaults, product-row state, Notification Health, Calendar/assignment, Volunteer, volunteer-schedule compatibility, and zero residue. Migration `20260812123430` makes the direct/default posture explicit; only approved staging advanced from `20260811123300` to `20260812123430`, where hosted metadata, type parity, Notification Health, and zero residue passed. Full independent technical recovery is proven. Production remains at `20260714122230`; production migration is separately reviewed and unperformed. Recurring scheduling/failure notification, recovery/rollback ownership, application-driven Initial email, real Bozeman provisioning, and controlled pilot remain blocking. Application email is disabled and launch remains `NO-GO`.
36. `12.35 Windows Recurring Backup Operations`
   - Registers one enabled current-operator Interactive Task Scheduler task daily at `03:15` local time with `StartWhenAvailable`, exact production locks, failure notification, and no database credential/private age identity in arguments. A scheduled pre-network fixture failed as designed with atomic credential-free status, no artifact or production connection, and a Windows notification Jelani confirmed visible; its temporary task/status were removed. Jelani/the Project Local product-operator owner now owns backup operations/failure response, restore and Vercel rollback approval/execution, pause, email disablement, and pilot cancellation. The single authorized scheduled production attempt failed safely at `migration_preflight_failed` and produced no new artifact, so scheduled success/checksum/retention remains blocking. Production remains recorded at `20260714122230`, application email remains disabled, and launch remains `NO-GO`.
37. `12.35.11 Controlled One-Shot Scheduled Production Backup Proof`
   - Runs exactly one triggerless temporary-task production execution through the reviewed action/principal boundary. Read-only preflight, five credential-safe native dump stages, the unchanged six-file package, age encryption, `62622`-byte daily/weekly publication, independently matching SHA-256, Sunday promotion, retention, duplicate prevention, and plaintext cleanup pass. The temporary task is removed; the permanent task does not execute.
38. `12.35.12 Backup/Recovery Evidence Reconciliation and Recurring Task Decision`
   - Reconciles backup/recovery as complete and non-blocking. A harmless disabled daily clone proves that enabling after a missed occurrence does not catch up on this Windows host. The unchanged permanent daily `03:15` task is enabled without execution and remains `Ready` for its next future occurrence. Overall launch remains `NO-GO` for production migrations/first health check, application-driven Initial email, real provisioning, controlled pilot, and explicit approval.
39. `12.36.5 Controlled Established-Production Migration Application`
   - Completed the authorized production preflight/apply/postflight on `project-local-production` (`wdlaauzknfggoqldolmx`) through exactly `20260811123300` and `20260812123430`. Exact history, types, Notification Health metadata, privileges/defaults, RLS/FORCE RLS, ownership/platform posture, one existing Auth identity, and zero product/storage/fixture state passed. The permanent task is enabled/Ready at lock `20260812123430`. Production migration is no longer a launch blocker; application email, real provisioning, first authorized Notification Health execution, controlled pilot, and approval remain blocking.
40. `12.36 Established-Production Migration Gate Readiness`
   - Adds a separate exact-target, exact-opt-in, no-fixture production-capable gate without weakening the historical pristine/bootstrap gate. Disposable local proof starts at `20260714122230`, preserves a legitimate synthetic Auth identity and minimal product fixture, applies exactly `20260811123300` then `20260812123430`, and proves history, types, Notification Health, direct/default privileges, RLS/FORCE RLS, owner/platform posture, and zero residue. It also locally proves the fail-closed future backup-task lock transition while leaving the enabled/Ready permanent task unchanged at `20260714122230`. Production/staging were not contacted; production migration remains blocking and launch remains `NO-GO`.
41. `12.37 Persisted Tasks Page Cutover and Reusable Task Creation`
...
43. `12.40 Persisted Assignment Detail Product Entry and Admin Drill-Down`
   - Promotes the existing capability-gated persisted assignment detail route from Calendar roster rows and exact Needs Attention pending/denied assignments. Multiple affected assignments remain separate destinations. The detail page joins the current Project Local admin shell and preserves its narrow read boundary and unavailable state. Response-link reveal/copy, token issuance, response mutation, edit workflow, broad assignment directory/search, and Volunteer-page entry remain disabled or deferred. The slice adds no query/RPC/migration/type/hosted/email/backup change or real Bozeman data; local desktop/mobile captures await product-owner review and launch remains `NO-GO`.
44. `12.41 First Real Bozeman Production Workspace and Admin Provisioning`
   - Uses the reviewed 12.14 operator boundary to create exactly one active `bozeman` workspace, one active project contact for the single approved Auth identity, and one active canonical `mainScheduler` grant. Authenticated RLS resolves exactly that one workspace/contact/grant; every other product/storage count remains zero. Production stays at `20260812123430`, application email stays disabled, Notification Health is not invoked, and launch remains `NO-GO` pending controlled email/health proof and pilot approval.
...
- 12.15 Manual Volunteer Profile Add/Edit Permanent Path. Completed as the first permanent volunteer-management product path for the Bozeman beta. `/admin/volunteers` now reads persisted `volunteer_profiles`, supports authorized manual create/edit through `create_manual_volunteer_profile` and `update_volunteer_profile_manual_fields`, preserves questionnaire provenance, denies direct authenticated table writes, and avoids mock volunteer fallback/mixing. This slice added migration `20260714121500_manual_volunteer_profiles.sql` and generated type changes, so hosted staging validation is required before the new DB/RPC behavior is trusted for any hosted beta dependency.
- 12.15.1 Hosted Staging Migration + Volunteer Profile Management Validation Gate. Completed against non-production `project-local-staging` (`kfuujcfxoayukywvtaeh`). The approved staging project was reactivated, verified as `ACTIVE_HEALTHY`, and advanced from `20260705000000` to `20260714121500`. `npm run test:volunteer-profile-management:hosted` passed with the exact opt-in, proving generated public-schema type parity, manual create/edit RPC behavior, questionnaire provenance compatibility, RLS/capability isolation, grant lifecycle, direct table-write denial, malformed/protected input rejection, safe errors, and hosted disposable cleanup with zero residue. The resume refreshed committed generated types from hosted output, aligned the volunteer RPC caller with generated optional-argument typing, and narrowed the hosted static service-role guard without adding any product route behavior. Next work may proceed to `12.16 Calendar Create/Edit Scheduled Item Implementation` if local compatibility remains green.
- 12.16 Calendar Create/Edit Scheduled Item Implementation. Completed as the first permanent `/admin/calendar` scheduled-item write path. Authorized project contacts with effective `calendar.edit` can create and edit persisted one-off timed Calendar items from the existing Calendar UI; read-only contacts can view but cannot mutate. The slice adds migration `20260714121600_calendar_item_management.sql`, nullable `follow_up_project_contact_id`, `0..99` timed/date-based needed counts, a create RPC that derives Follow-up Contact from the authenticated scheduler, and an allowlisted `update_calendar_item_one_off_timed` RPC. It preserves server-owned workspace/contact/capability derivation, no mock Calendar fallback/mixing, assignment-derived coverage truth, and direct table-write denial. Task-preset selector, all-day/date-based authoring, archive/delete, publication, assignment picker/mutations, delivery, public lookup, remembered devices, response-link activation, and production data remain out of scope.
- 12.16.1 Hosted Staging Calendar Item Management Validation Gate. Completed against non-production `project-local-staging` (`kfuujcfxoayukywvtaeh`). The exact-opt-in gate `npm run test:calendar-item-management:hosted` advanced staging from `20260714121500` to `20260714121600`, compared hosted generated public-schema types to committed types, validated one-off timed create/edit, server-derived Follow-up Contact, zero-needed `0/0 assigned` read-model behavior, protected-field preservation, view-only and missing-`calendar.edit` denial, cross-contact/cross-workspace isolation, revoked/expired/inactive grants, role/title non-authorization, direct table-write denial, malformed schedule/source rejection, fake preset rejection, existing preset-backed source compatibility, existing-row nullable Follow-up Contact compatibility, safe output, and exact-run plus namespace zero residue. The gate added no product/runtime route code, extra migration, service-role application path, real data, or Calendar feature expansion. Next work may proceed to `12.17 Calendar Task Preset Selector and One-Off Definition Path` if local compatibility remains green.
- 12.17 Calendar Task Preset Selector and One-Off Definition Path. Completed locally as the first persisted task-preset source path inside `/admin/calendar`, not a `/admin/tasks` cutover. The Calendar route now reads active same-workspace task presets through a server-only explicit-selector seam when the authenticated deterministic workspace context also has `tasks.view`; it no longer uses mock task presets as Calendar creation truth. Authorized `calendar.edit` contacts can create timed Calendar items from either a real persisted preset or a custom one-off source. One-off timed edits continue through `update_calendar_item_one_off_timed`; preset-backed timed edits use the new allowlisted `update_calendar_item_preset_timed` RPC and preserve preset source, Follow-up Contact, workspace, lifecycle, and assignment truth. The read model keeps occurrence title snapshots as display labels while carrying task preset id/label as safe source metadata. `npm run test:calendar-source-selection` proves local disposable selector/create/read/edit/isolation/direct-write-denial/zero-residue behavior, and `npm run test:calendar` was extended to exercise real preset selection and preset-backed create/reload/edit/reload. The required hosted validation for migration `20260714121700_calendar_source_selection.sql`, generated types, and RPC behavior is recorded in the completed 12.17.1 gate below.
- 12.17.1 Hosted Staging Calendar Source Selection Validation Gate. Completed against non-production `project-local-staging` (`kfuujcfxoayukywvtaeh`). The exact-opt-in gate `npm run test:calendar-source-selection:hosted` verified the project as `ACTIVE_HEALTHY`, confirmed staging at migration `20260714121700` before and after the successful run, compared hosted generated public-schema types to committed types, and validated selector authorization/projection/scoping, active-only lifecycle eligibility, missing-`tasks.view` selector unavailability with custom one-off continuity, role/title non-authorization, preset-backed timed create/edit, source/snapshot semantics, fake/wrong-workspace/archived preset rejection, mixed-source rejection, source immutability/no conversion, mutation capability enforcement, cross-contact/cross-workspace isolation, revoked/expired/inactive grant denial, direct table-write denial for Calendar items and task presets, read-model compatibility, existing one-off compatibility, existing preset-backed row compatibility, malformed input rejection, safe output, and exact-run plus namespace zero residue with hosted disposable residue count `0`. Only hosted harness fixture values were corrected to obey existing constraints; no product/runtime/schema behavior changed. Next work may proceed to `12.18 Volunteer Assignment Picker and Create/Cancel Commands` if local compatibility remains green.
- 12.19 Draft/Private Versus Published/Live Calendar Visibility. Completed locally as the first persisted Calendar publication boundary. New Calendar items default to private drafts owned by the creating project contact, published items become visible to authorized same-workspace project contacts, and `publish_calendar_item` records server-derived published metadata without sending email, exposing volunteer schedules, issuing response tokens, or activating response-link reveal/copy UI. Draft assignments may be prepared only by the draft creator; response-token/read/submit/reveal boundaries fail closed for drafts. The slice adds migration `20260714121900_calendar_publication_visibility.sql`, generated public-schema type changes, updated Calendar/assignment/response functions, safe read-model publication projection, and `npm run test:calendar-publication-visibility`.
- 12.19.1 Hosted Staging Calendar Publication Visibility Validation Gate. Completed against non-production `project-local-staging` (`kfuujcfxoayukywvtaeh`). The exact-opt-in gate `npm run test:calendar-publication-visibility:hosted` advanced staging from `20260714121800` to `20260714121900` during its first migration application after a safe unapplied migration comment typo fix; the final successful run verified migration level `20260714121900` before and after validation. Hosted generated public-schema type parity passed after a UTF-8-safe generated type refresh. The gate proved publication defaults, legacy draft fail-closed behavior, creator-only draft reads, published cross-contact reads, safe projection, publish authorization/idempotency, assignment/token/public-response gating, direct table-write denial, capability and workspace/contact isolation, malformed input rejection, no email/delivery/public-lookup/remembered-device coupling, safe output, exact-run cleanup, namespace zero residue, and hosted disposable residue count `0`. No product/runtime code changed during the gate. Next work may proceed to `12.20 Secure Account-Light Volunteer Schedule Access` if local compatibility remains green.
- 12.20 Secure Account-Light Volunteer Schedule Access. Completed locally as the first dedicated volunteer-facing schedule access path. Authorized contacts with effective `assignments.edit` can issue/revoke hash-only volunteer schedule access credentials; `/v/access/[token]` exchanges the bearer for a session-only HttpOnly cookie; and `/v/schedule` reads only that volunteer's published assignments. The slice adds migration `20260714122000_volunteer_schedule_access.sql`, generated public-schema type changes, dedicated RPCs, safe route/cookie handling, local DB validation, and local browser proof. Confirm/Deny remained out of scope until 12.21; email, remembered devices, public lookup, `/v/demo` cutover, response-link reveal/copy activation, and real Bozeman data remain out of scope.
- 12.20.1 Hosted Staging Volunteer Schedule Access Validation Gate. Completed against non-production `project-local-staging` (`kfuujcfxoayukywvtaeh`). The exact-opt-in gate `npm run test:volunteer-schedule-access:hosted` verified the target as `ACTIVE_HEALTHY`, verified migration level `20260714122000` before and after validation, refreshed generated public-schema types from hosted output with UTF-8-safe generation, and proved hosted token separation, hash-only storage, TTL policy, issuance/revocation authorization, public schedule ready/empty/unavailable states, published-assignment filtering, response truth, safe projection, direct table denial, cross-contact/cross-workspace/grant lifecycle isolation, role/title non-authorization, multiple credentials, last-used behavior, hosted route exchange, clean URL, cookie attributes, host preservation, Not-you clearing, 390px layout, focus behavior, safe output, exact-run cleanup, namespace zero residue, and hosted disposable residue count `0`. No product/runtime code changed during the gate. Next work may proceed to `12.21 Volunteer Confirm/Deny Round Trip` if local compatibility remains green.
- 12.21 Volunteer Confirm/Deny Round Trip. Completed locally as the first persisted volunteer response round trip from account-light `/v/schedule`. The slice adds migration `20260714122100_volunteer_schedule_responses.sql`, generated public-schema type changes, `submit_volunteer_schedule_assignment_response`, `confirm_all_volunteer_schedule_assignments`, a tightened `submit_assignment_response_by_token`, and safe server actions that read only the HttpOnly schedule cookie. Volunteers can confirm eligible assignments, mark eligible assignments as `Can’t make it` with an optional normalized note, change confirmed to declined only outside the 48-hour cutoff, change declined to confirmed before start, and Confirm All pending eligible assignments. Schedule-cookie mutations record `response_source = 'volunteer_schedule'`; the assignment-response-token route remains `public_token`. The database derives the 48-hour cutoff/start lock from persisted Calendar item schedule/timezone and server time. Assignment/current-response rows remain the only truth; no email, public lookup, remembered devices, `/v/demo` cutover, response-link reveal/copy, assignment-detail links, Calendar counters, or service-role product path was added. Local DB/browser validation and hosted 12.21.1 staging validation passed with zero residue.
