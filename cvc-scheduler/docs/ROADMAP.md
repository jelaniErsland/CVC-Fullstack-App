...
9. `12.19 Draft/Private Versus Published/Live Calendar Visibility`
   - Completed locally: persisted Calendar items now default to private drafts owned by the creator, publish through a one-way server boundary, and separate visibility from email/public response-link/volunteer schedule activation. Hosted validation is required in 12.19.1.
