...
13. `12.21 Volunteer Confirm/Deny Round Trip`
   - Completed locally: `/v/schedule` can persist Confirm, Can’t make it with optional notes, reversible eligible response changes, and Confirm All through server-owned credential-scoped RPCs; `/respond/[token]` now shares the same 48-hour/start-lock mutation policy. Hosted validation is pending because migration/RPC/generated types changed.
14. `12.21.1 Hosted Staging Volunteer Response Round-Trip Validation Gate`
   - Required next before beta use of the 12.21 response boundary.
