...
- Secure account-light volunteer schedule access is implemented and hosted-validated through 12.20.1. `/v/demo` remains mock and separate; `/respond/[token]` remains single-assignment; Confirm/Deny schedule integration remains the next beta-critical gap.
...
Product/runtime code did not change during the gate. Confirm/Deny, Confirm All, email, remembered devices, public lookup, `/v/demo` cutover, response-link reveal/copy activation, and real Bozeman/Belgrade data remain out of scope.
