...
- Calendar task-preset selection, the first Calendar volunteer assignment picker/create/cancel path, the draft/private versus published/live Calendar visibility boundary, secure account-light volunteer schedule access, and volunteer Confirm/Deny from `/v/schedule` are implemented and hosted-validated through 12.21.1. Initial assignment email delivery is implemented locally in 12.22 with hosted validation still required before hosted beta use. Communications persistence, public lookup, remembered devices, and response-link admin reveal/copy activation remain unimplemented or intentionally paused.
...
16. `12.22.1 Hosted Staging Initial Assignment Notification Validation Gate`
   - Required before hosted beta use because 12.22 adds migration `20260714122200_initial_assignment_notifications.sql`, new authenticated RPCs, generated public-schema types, and hosted-affecting email-delivery metadata behavior.
...
- Basic initial assignment email delivery now has a local 12.22 boundary: explicit admin send only, server-derived recipient and Follow-up Contact eligibility, duplicate-send prevention, hash-only schedule-access credential issuance, safe delivery metadata, and a disabled-by-default recording transport. Hosted staging validation remains a beta blocker before relying on the new migration/RPC/type boundary in hosted environments.
