...
- Calendar task-preset selection, the first Calendar volunteer assignment picker/create/cancel path, the draft/private versus published/live Calendar visibility boundary, and secure account-light volunteer schedule access are implemented and hosted-validated through 12.20.1. Volunteer Confirm/Deny from `/v/schedule` is implemented locally in 12.21 and requires hosted validation before beta use. Initial assignment email delivery, Communications persistence, public lookup, remembered devices, and response-link admin reveal/copy activation remain unimplemented or intentionally paused.
...
13. `12.21 Volunteer Confirm/Deny Round Trip`
   - Completed locally: account-light volunteers can Confirm, mark `Can’t make it` with optional notes, reverse eligible responses, and Confirm All through persisted assignment/current-response truth. Hosted staging validation is required next for migration `20260714122100`.
14. `12.21.1 Hosted Staging Volunteer Response Round-Trip Validation Gate`
   - Required next before beta use.
...
- Secure account-light volunteer schedule access is implemented and hosted-validated through 12.20.1. `/v/demo` remains mock and separate. 12.21 locally integrates Confirm/Deny and Confirm All into `/v/schedule`, and `/respond/[token]` now shares the same 48-hour/start-lock mutation policy; hosted 12.21.1 remains required before beta use.
...
Product/runtime code did not change during the 12.20.1 gate. Confirm/Deny and Confirm All were added later in the local 12.21 slice and require hosted 12.21.1 validation. Email, remembered devices, public lookup, `/v/demo` cutover, response-link reveal/copy activation, and real Bozeman/Belgrade data remain out of scope.

Recommended next slice after the successful hosted gate: `12.21 Volunteer Confirm/Deny Round Trip`.
