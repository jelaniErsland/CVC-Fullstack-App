...
export const bozemanBetaAuditDomains: readonly BetaDomain[] = [
  ...
  },
  {
    ...
    repositoryFindings: [
      ...
      "12.37 makes the 12.13 read-model contract route-used",
      "/admin/tasks uses persisted task preset truth with no mock fallback",
      "tasks.view reads and tasks.edit create/archive are server-scoped",
      "Calendar uses the same active persisted task presets while existing occurrences retain snapshots",
    ],
    betaNeed:
      "Preserve Task preset = reusable definition and Scheduled item = Calendar occurrence; keep general preset editing deferred until a separate reviewed boundary exists.",
    blockers: [],
    deferrable: [
      "general_task_preset_update_ui",
      "advanced_custom_field_authoring",
      "task_duplicate_ui",
